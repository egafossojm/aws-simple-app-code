...
class StatementValidator implements StatementValidatorInterface
{
    ...
    {
        if ($statement->getAuthnInstantTimestamp() == false) {
            throw new LightSamlValidationException('AuthnStatement MUST have an AuthnInstant attribute');
        }
        if (Helper::validateOptionalString($statement->getSessionIndex()) == false) {
        ...
        if ($statement->getSubjectLocality()) {
            if (Helper::validateOptionalString($statement->getSubjectLocality()->getAddress()) == false) {
            ...
            if (Helper::validateOptionalString($statement->getSubjectLocality()->getDnsName()) == false) {
        ...
        if ($statement->getAuthnContext() == false) {
    ...
    {
        if ($authnContext->getAuthnContextClassRef() == false &&
            $authnContext->getAuthnContextDecl() == false &&
            $authnContext->getAuthnContextDeclRef() == false
        ) {
        ...
        if ($authnContext->getAuthnContextClassRef()) {
            if (Helper::validateWellFormedUriString($authnContext->getAuthnContextClassRef()) == false) {
        ...
        if ($authnContext->getAuthnContextDeclRef()) {
            if (Helper::validateWellFormedUriString($authnContext->getAuthnContextDeclRef()) === false) {
    ...
    {
        if ($statement->getAllAttributes() == false) {
    ...
    {
        if (Helper::validateRequiredString($attribute->getName()) == false) {
