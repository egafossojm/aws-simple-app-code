...
class WPSEO_Option_InternalLinks extends WPSEO_Option
{
    ...
    protected function clean_option($option_value, $current_version = null, $all_old_option_values = null)
    {
        ...
        if (isset($current_version) && version_compare($current_version, '1.5.2.3', '<')) {
            if (has_action('init', ['WPSEO_Options', 'bring_back_breadcrumb_defaults']) === false) {
                add_action('init', ['WPSEO_Options', 'bring_back_breadcrumb_defaults'], 3);
            }
        }
            ...
            foreach ($option_value as $key => $value) {
                ...
                switch ($switch_key) {
                    ...
                    case 'post_types-':
                        ...
                        if ($taxonomies !== [] && in_array($value, $taxonomies, true)) {
                        ...
                        /* 'taxonomy-' . $tax->name . '-ptparent' fields. */
                    case 'taxonomy-':
                        if ($allowed_post_types !== [] && in_array($value, $allowed_post_types, true)) {
                            $option_value[$key] = $value;
                        } elseif ((string) $value === '0' || (string) $value === '') {
                            $option_value[$key] = 0;
