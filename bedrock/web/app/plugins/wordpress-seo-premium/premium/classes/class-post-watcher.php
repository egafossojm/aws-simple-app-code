...
class WPSEO_Post_Watcher extends WPSEO_Watcher
{
    ...
    public function page_scripts()
    {
        ...
        if ($pagenow === 'edit.php' || $this->is_nested_pages($pagenow)) {
            wp_enqueue_script('wp-seo-premium-quickedit-notification', plugin_dir_url(WPSEO_PREMIUM_FILE).'assets/js/dist/wp-seo-premium-quickedit-notification-'.$version.WPSEO_CSSJS_SUFFIX.'.js', ['jquery'], WPSEO_VERSION);
            wp_localize_script('wp-seo-premium-quickedit-notification', 'wpseoPremiumStrings', WPSEO_Premium_Javascript_Strings::strings());
        }

        if ($pagenow === 'post.php') {
            wp_enqueue_script('wp-seo-premium-redirect-notifications', plugin_dir_url(WPSEO_PREMIUM_FILE).'assets/js/dist/wp-seo-premium-redirect-notifications-'.$version.WPSEO_CSSJS_SUFFIX.'.js', ['jquery'], WPSEO_VERSION);
    ...
    protected function set_hooks()
    {
        ...
        add_action('edit_form_advanced', [$this, 'old_url_field'], 10, 1);
        add_action('edit_page_form', [$this, 'old_url_field'], 10, 1);
