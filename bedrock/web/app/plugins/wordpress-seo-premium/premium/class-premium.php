...
class WPSEO_Premium
{
    ...
    public function __construct()
    {
        ...
        $this->integrations = [
            ...
            'orphaned-post-notifier' => new WPSEO_Premium_Orphaned_Post_Notifier(['post', 'page'], Yoast_Notification_Center::get()),
        ];
    ...
    private function setup()
    {
        ...
        if (is_admin()) {
            ...
            add_action('admin_init', [$this, 'init_helpscout_support'], 1);
            ...
            global $pagenow;
            if (in_array($pagenow, ['post-new.php', 'post.php', 'edit.php'], true)) {
                new WPSEO_Custom_Fields_Plugin;
            }

            add_action('admin_init', [$this, 'initialize_tracking'], 1);
            ...
            add_action('wpseo_admin_page_meta_post_types', [
                $this,
                'admin_page_meta_post_types_checkboxes',
            ], 10, 2);

            // Add page analysis fields to variable array key patterns.
            add_filter('wpseo_option_titles_variable_array_key_patterns', [
                $this,
            ...
            add_action('wpseo_licenses_forms', [$license_manager, 'show_license_form']);
            ...
            if (get_option('permalink_structure')) {
                ...
                if (isset($redirect_created) && $redirect_created !== false) {
                    ...
                    add_action('all_admin_notices', [$message, 'display']);
                }
            }
        } else {
        ...
        add_action('admin_init', [$this, 'enqueue_multi_keyword']);
        add_action('admin_init', [$this, 'enqueue_social_previews']);
    ...
    {
        if (in_array('page-analyse-extra-', $patterns, true) !== true) {
    ...
    public function add_submenu_pages($submenu_pages)
    {
        ...
        $submenu_pages[] = [
            ...
            apply_filters_deprecated('wpseo_premium_manage_redirects_role', ['wpseo_manage_redirects'], 'WPSEO 5.5', false, 'Use the introduced wpseo_manage_redirects capability instead.'),
            'wpseo_redirects',
    ...
    public function init_helpscout_support()
    {
        ...
        if (WPSEO_Metabox::is_post_edit($GLOBALS['pagenow']) || $is_beacon_page) {
            add_action('admin_enqueue_scripts', [$this, 'enqueue_contact_support']);
    ...
    public function enqueue_contact_support()
    {
        ...
        wp_enqueue_script('yoast-contact-support', plugin_dir_url(WPSEO_PREMIUM_FILE).'assets/js/dist/wpseo-premium-contact-support-'.$version.WPSEO_CSSJS_SUFFIX.'.js', ['jquery'], WPSEO_VERSION);
    }
