 ...
 */
$taxonomies = apply_filters('wpseo_sitemaps_supported_taxonomies', get_taxonomies(['public' => true], 'objects'));
if (is_array($taxonomies) && $taxonomies !== []) {
    foreach ($taxonomies as $tax) {
        // Explicitly hide all the core taxonomies we never want in our sitemap.
        if (in_array($tax->name, ['link_category', 'nav_menu'], true)) {
            continue;
        }

        $title_options = WPSEO_Options::get_option('wpseo_titles');

        if ($tax->name === 'post_format' && ! empty($title_options['disable-post_format'])) {
            continue;
        }
