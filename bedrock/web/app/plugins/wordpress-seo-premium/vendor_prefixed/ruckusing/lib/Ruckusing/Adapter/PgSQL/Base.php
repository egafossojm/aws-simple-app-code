...
class Ruckusing_Adapter_PgSQL_Base extends \YoastSEO_Vendor\Ruckusing_Adapter_Base implements \YoastSEO_Vendor\Ruckusing_Adapter_Interface
{
    ...
    {
        $types = ['primary_key' => ['name' => 'serial'], 'string' => ['name' => 'varchar', 'limit' => 255], 'text' => ['name' => 'text'], 'tinytext' => ['name' => 'text'], 'mediumtext' => ['name' => 'text'], 'integer' => ['name' => 'integer'], 'tinyinteger' => ['name' => 'smallint'], 'smallinteger' => ['name' => 'smallint'], 'mediuminteger' => ['name' => 'integer'], 'biginteger' => ['name' => 'bigint'], 'float' => ['name' => 'float'], 'decimal' => ['name' => 'decimal', 'scale' => 0, 'precision' => 10], 'datetime' => ['name' => 'timestamp'], 'timestamp' => ['name' => 'timestamp'], 'time' => ['name' => 'time'], 'date' => ['name' => 'date'], 'binary' => ['name' => 'bytea'], 'tinybinary' => ['name' => 'bytea'], 'mediumbinary' => ['name' => 'bytea'], 'longbinary' => ['name' => 'bytea'], 'boolean' => ['name' => 'boolean'], 'tsvector' => ['name' => 'tsvector'], 'uuid' => ['name' => 'uuid']];
    ...
    public function create_schema_version_table()
    {
        if (! $this->has_table($this->get_schema_version_table_name())) {
            ...
            $t->finish();
            $this->add_index($this->get_schema_version_table_name(), 'version', ['unique' => \true]);
        }
    }
...
SQL;
     ...
     * @param  bool  $reload_tables  reload table or not
     * @return bool
     */
...
SQL;
    ...
    public function add_index($table_name, $column_name, $options = [])
    {
        ...
        $sql = \sprintf('CREATE %sINDEX %s ON %s(%s)', $unique ? 'UNIQUE ' : '', $this->quote_column_name($index_name), $this->quote_column_name($table_name), \implode(', ', $cols));
...
SQL;
        ...
        foreach ($result as $row) {
            $indexes[] = ['name' => $row['relname'], 'unique' => $row['indisunique'] == 't' ? \true : \false];
        }
...
SQL;
        ...
        $primary_keys = [];
        foreach ($result as $row) {
            $primary_keys[] = ['name' => $row['attname'], 'type' => $row['format_type']];
        }
