...
class Ruckusing_Adapter_Sqlite3_Base extends \YoastSEO_Vendor\Ruckusing_Adapter_Base implements \YoastSEO_Vendor\Ruckusing_Adapter_Interface
{
    ...
    {
        $types = ['primary_key' => ['name' => 'integer'], 'string' => ['name' => 'varchar', 'limit' => 255], 'text' => ['name' => 'text'], 'tinytext' => ['name' => 'text'], 'mediumtext' => ['name' => 'text'], 'integer' => ['name' => 'integer'], 'tinyinteger' => ['name' => 'smallint'], 'smallinteger' => ['name' => 'smallint'], 'mediuminteger' => ['name' => 'integer'], 'biginteger' => ['name' => 'bigint'], 'float' => ['name' => 'float'], 'decimal' => ['name' => 'decimal', 'scale' => 0, 'precision' => 10], 'datetime' => ['name' => 'datetime'], 'timestamp' => ['name' => 'datetime'], 'time' => ['name' => 'time'], 'date' => ['name' => 'date'], 'binary' => ['name' => 'blob'], 'tinybinary' => ['name' => 'blob'], 'mediumbinary' => ['name' => 'blob'], 'longbinary' => ['name' => 'blob'], 'boolean' => ['name' => 'boolean']];
    ...
    public function add_column($table_name, $column_name, $type, $options = [])
    {
        ...
        $defaultOptions = ['limit' => null, 'precision' => null, 'scale' => null];
    ...
    public function indexes($table_name)
    {
        ...
        foreach ($result as $row) {
            $indexes[] = ['name' => $row['name'], 'unique' => $row['unique'] ? \true : \false];
        }
    ...
    public function primary_keys($table)
    {
        ...
        foreach ($result as $row) {
            if ($row['pk']) {
                $primary_keys[] = ['name' => $row['name'], 'type' => $row['type']];
            }
        }
