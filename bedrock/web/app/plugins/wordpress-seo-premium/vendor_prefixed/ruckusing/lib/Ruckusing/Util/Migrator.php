...
class Ruckusing_Util_Migrator
{
     ...
     * @param  bool  $use_cache  the current logger
     * @return array
     */
    ...
    public static function get_migration_files($directories, $direction)
    {
            ...
            if ($file_cnt > 0) {
                for ($i = 0; $i < $file_cnt; $i++) {
                    if (\preg_match('/^(\\d+)_(.*)\\.php$/', $files[$i], $matches)) {
                        if (\count($matches) == 3) {
                            $valid_files[] = ['name' => $files[$i], 'module' => $name];
                        }
                    }
                }
            }
        }
        \usort($valid_files, ["\YoastSEO_Vendor\Ruckusing_Util_Migrator", 'migration_compare']);
        //sorts in place
        if ($direction == 'down') {
        ...
        for ($i = 0; $i < $cnt; $i++) {
            ...
            if (\preg_match('/^(\\d+)_(.*)\\.php$/', $migration['name'], $matches)) {
                $files[] = ['version' => $matches[1], 'class' => $matches[2], 'file' => $matches[0], 'module' => $migration['module']];
            }
        }
     ...
     * @param  bool  $only_index  whether to only return the index of the version
