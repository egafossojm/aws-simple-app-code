...
class WPSEO_Frontend
{
    ...
    protected function __construct()
    {
        ...
        add_action('template_redirect', [$this, 'noindex_feed']);
    ...
    private function generate_title($title, $separator_location)
    {
        ...
        if (defined('ICL_LANGUAGE_CODE') && strpos($title, ICL_LANGUAGE_CODE) !== false) {
    ...
    public function robots()
    {
        ...
        if (is_object($post) && $this->frontend_page_type->is_simple_page()) {
            $private = $post->post_status === 'private';
            ...
            if (is_search() || is_404()) {
                ...
                if (is_string($term_meta) && $term_meta !== 'default') {
        ...
        if ((string) get_option('blog_public') === '0' || isset($_GET['replytocom'])) {
            $robots['index'] = 'noindex';
        }
    ...
    public function adjacent_rel_links()
    {
         ...
         */
        if (apply_filters('wpseo_disable_adjacent_rel_links', false) === true) {
            return;
        }
     ...
     *
     * @return bool False when no redirect was triggered.
     */
     ...
     * @codeCoverageIgnore
     *
     * @return bool Boolean indicating whether the publisher link was printed.
     */
