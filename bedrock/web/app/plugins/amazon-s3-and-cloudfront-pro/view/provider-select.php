...
<div class="as3cf-content as3cf-provider-select">
	<?php
    if (! empty($_GET['action']) && $_GET['action'] === 'change-provider') {
...
?>
    ...
    foreach ($providers as $provider_key => $provider_class) {
        ...
        $defined_constants = [];
        foreach ([$key_constant, $secret_constant, $keys_settings_constant] as $defined_constant) {
            if ($defined_constant) {
        ...
        echo $define_authmethod_attr; ?>>
						...
						<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="define"<?php echo $selected_authmethod !== 'define' ? ' style="display: none"' : ''; ?>>
							<td></td>
							<td>
								<?php
        if ($any_access_key_constant_defined) {
            ...
            $defined_constants_str = implode($multiple_defined_keys_glue, $defined_constants);
    ...
    'secret-access-key' => '**************************************',
        ...
        ?>
                        ...
                        if ($provider_class::use_server_roles_allowed()) {
							...
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="server-role"<?php echo $selected_authmethod !== 'server-role' ? ' style="display: none"' : ''; ?>>
								<td></td>
								<td>
...
define( 'AS3CF_AWS_USE_EC2_IAM_ROLE', true );
        ...
        echo $db_authmethod_attr; ?>>
						...
						<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="db"<?php echo $selected_authmethod !== 'db' ? ' style="display: none"' : ''; ?>>
							<td></td>
							<td>
								<?php
