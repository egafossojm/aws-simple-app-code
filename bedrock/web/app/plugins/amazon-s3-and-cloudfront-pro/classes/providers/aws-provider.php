...
class AWS_Provider extends Provider
{
     ...
     * @return array
     */
    public function list_buckets(array $args = [])
    {
     ...
     * @return bool
     */
    public function does_object_exist($bucket, $key, array $options = [])
    {
     ...
     * @return string
     */
    public function get_object_url($bucket, $key, $expires, array $args = [])
    {
     ...
     * @return array
     */
    public function list_objects(array $args = [])
    {
    ...
    public function list_keys(array $locations)
    {
        ...
        $results = CommandPool::batch($this->s3_client, $commands, ['preserve_iterator_keys' => true]);
    ...
    public function can_write($bucket, $key, $file_contents)
    {
        try {
            ...
            if (! $e instanceof S3Exception || $e->getAwsErrorCode() !== 'AccessDenied') {
                return $e->getMessage();
            }
        }
    ...
    protected function url_prefix($region = '', $expires = null)
    {
        ...
        if ($region !== '') {
            $delimiter = '-';
            if ($region == 'eu-central-1' && ! is_null($expires)) {
     ...
     * @return string
     */
    protected function url_domain($domain, $bucket, $region = '', $expires = null, $args = [], $preview = false)
    {
        if ($args['domain'] === 'cloudfront' && is_null($expires) && $args['cloudfront']) {
            ...
            $domain = $cloudfront;
        } elseif ($args['domain'] === 'virtual-host') {
            $domain = $bucket;
        } elseif ($args['domain'] === 'path' || $args['force-https']) {
