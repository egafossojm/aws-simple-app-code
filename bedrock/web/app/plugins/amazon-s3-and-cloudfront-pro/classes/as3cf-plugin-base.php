...
abstract class AS3CF_Plugin_Base
{
    ...
    public function get_defined_settings($force = false)
    {
        ...
        if (is_null($this->defined_settings) || $force) {
            ...
            $unserialized = is_array($unserialized) ? $unserialized : [];
    ...
    {
        if (static::settings_constant() !== false && ! has_action('update_site_option_'.'as3cf_constant_'.static::settings_constant(), [
            $this,
            'settings_constant_changed',
        ])) {
            add_action('add_site_option_'.'as3cf_constant_'.static::settings_constant(), [
                $this,
                'settings_constant_added',
            ], 10, 3);
            add_action('update_site_option_'.'as3cf_constant_'.static::settings_constant(), [
                $this,
                'settings_constant_changed',
            ], 10, 4);
        }
    ...
    {
        $db_settings = get_site_option(static::SETTINGS_KEY, []);
     ...
     */
    public function remove_defined_setting($key)
    {
     ...
     * @return string
     */
    public function get_plugin_page_url($args = [], $url_method = 'network', $escape = true)
    {
        $default_args = [
     ...
     * @return string
     */
    public function get_plugin_action_settings_text()
    {
     ...
     * @param  bool  $footer
     */
    public function enqueue_script($handle, $path, $deps = [], $footer = true)
    {
     ...
     * @param  array  $deps
     */
    public function enqueue_style($handle, $path, $deps = [])
    {
        $version = $this->get_asset_version();
     ...
     * @return string
     */
    public function dbrains_url($path, $args = [], $hash = '')
    {
     ...
     * @return string
     */
    public function get_my_account_url($args = [], $hash = '')
    {
