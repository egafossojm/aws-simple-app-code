...
class AS3CF_Pro_Plugin_Installer
{
    ...
    public function __construct($process_key, $plugin_slug, $plugin_file_path)
    {
        ...
        add_action('wp_ajax_as3cfpro_install_plugins_'.$this->process_key, [$this, 'ajax_install_plugins']);
        add_action('admin_init', [$this, 'maybe_install_plugins']);
        add_action('admin_init', [$this, 'installer_redirect']);
        add_action('admin_notices', [$this, 'maybe_display_installer_notices']);
        add_action('network_admin_notices', [$this, 'maybe_display_installer_notices']);
    }
     ...
     */
    public function load_installer_assets()
    {
        ...
        wp_enqueue_script('as3cf-pro-installer', $src, ['jquery', 'wp-util'], $version, true);
    ...
    public function install_plugins()
    {
        ...
        if ($plugins_activated === count($this->plugins_to_install)) {
            ...
            $url = add_query_arg(['as3cfpro-install' => 1], network_admin_url('plugins.php'));
    ...
    protected function end_install($status)
    {
        ...
        $this->installer_notices['notices'][] = ['message' => $message, 'class' => $class];
    }
    ...
    public function maybe_display_installer_notices()
    {
        ...
        foreach ($notices['notices'] as $notice) {
            echo '<div class="as3cf-pro-installer-notice '.$notice['class'].'"><p>'.$notice['message'].'</p></div>';
        }
