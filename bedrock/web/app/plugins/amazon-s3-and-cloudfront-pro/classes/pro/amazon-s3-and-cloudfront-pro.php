...
class Amazon_S3_And_CloudFront_Pro extends Amazon_S3_And_CloudFront
{
    ...
    public function init($plugin_file_path)
    {
        ...
        // load assets
        add_action('as3cf_plugin_load', [$this, 'load_assets']);
    ...
    {
        add_action('load-upload.php', [$this, 'load_media_pro_assets'], 11);
        add_filter('as3cf_settings_tabs', [$this, 'settings_tabs']);
        add_action('as3cf_after_settings', [$this, 'settings_page']);

        // Pro customisations
        add_filter('as3cf_lost_files_notice', [$this, 'lost_files_notice']);
        add_action('as3cf_load_attachment_assets', [$this, 'load_attachment_js']);
        add_filter('as3cf_media_action_strings', [$this, 'media_action_strings']);

        // Media row actions
        add_filter('wp_prepare_attachment_for_js', [$this, 'enrich_attachment_model'], 10, 2);
        add_filter('bulk_actions-upload', [$this, 'add_list_table_bulk_actions']);
        add_filter('media_row_actions', [$this, 'add_media_row_actions'], 10, 2);
        add_action('admin_notices', [$this, 'maybe_display_media_action_message']);
        add_action('admin_init', [$this, 'process_media_actions']);

        // Ajax handlers
        add_action('wp_ajax_as3cfpro_process_media_action', [$this, 'ajax_process_media_action']);
        add_action('wp_ajax_as3cfpro_update_acl', [$this, 'ajax_update_acl']);
        ...
        // Diagnostic info
        add_filter('as3cf_diagnostic_info', [$this, 'diagnostic_info']);
    ...
    {
        $integrations = apply_filters('as3cf_integrations', [
    ...
    {
        $this->enqueue_style('as3cf-pro-styles', 'assets/css/pro/styles', ['as3cf-styles']);
        $this->enqueue_script('as3cf-pro-script', 'assets/js/pro/script', ['jquery', 'underscore']);

        wp_localize_script('as3cf-pro-script', 'as3cfpro', [
            'settings' => apply_filters('as3cfpro_js_settings', []),
            'strings' => apply_filters('as3cfpro_js_strings', []),
            'nonces' => apply_filters('as3cfpro_js_nonces', []),
        ]);
    ...
    public function load_media_pro_assets()
    {
        ...
        $this->enqueue_script('as3cf-pro-media-script', 'assets/js/pro/media', [
            'jquery',
            'media-views',
            'media-grid',
            'wp-util',
        ...
        wp_localize_script('as3cf-pro-media-script', 'as3cfpro_media', [
    ...
    {
        $this->enqueue_script('as3cf-pro-attachment-script', 'assets/js/pro/attachment', [
            'jquery',
            'wp-util',
        ], false);
        ...
        wp_localize_script('as3cf-pro-attachment-script', 'as3cfpro_media', [
            'strings' => [
     ...
     */
    public function render_view($view, $args = [])
    {
        extract($args);
     ...
     * @return mixed
     */
    public function get_all_provider_attachments($prefix, $count = false, $limit = false, $offset = 0)
    {
        global $wpdb;
     ...
     * @return string
     */
    public function get_custom_attachment_url($attachment, $args)
    {
    ...
    public function get_available_media_actions($scope = null)
    {
        ...
        if ($this->is_pro_plugin_setup(true)) {
            $actions['copy'] = ['singular', 'bulk'];
            $actions['download'] = ['singular', 'bulk'];
            $actions['update_acl'] = ['singular'];
        }

        $actions['remove'] = ['singular', 'bulk'];
    ...
    public function process_media_actions()
    {
        ...
        if (strpos($action, 'bulk_as3cfpro_') === false) {
            $available_actions = $this->get_available_media_actions('singular');
        ...
        if ($action === 'copy' && $id_count === 1 && ! empty($result) && 1 === ($result['count'] + $result['errors'])) {
    ...
    {
        global $pagenow;
        if (! in_array($pagenow, ['upload.php', 'post.php'])) {
            return;
        }
     ...
     * @throws Exception
     */
    public function download_attachment_from_provider($post_id, $force_new_provider_client = false, $skip_setup_check = false)
    {
     ...
     * @return int
     */
    public function get_media_library_provider_total($skip_transient = false)
    {
