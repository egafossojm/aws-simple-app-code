...
class AS3CF_Pro_Licences_Updates extends Delicious_Brains_API_Licences
{
    ...
    public function __construct(Amazon_S3_And_CloudFront_Pro $as3cf)
    {
        ...
        $plugin->purchase_url = $this->as3cf->dbrains_url('/wp-offload-media/pricing/', [
    ...
    {
        add_action('admin_notices', [$this, 'dashboard_licence_issue_notice']);
        add_action('network_admin_notices', [$this, 'dashboard_licence_issue_notice']);
        add_action('as3cf_pre_settings_render', [$this, 'licence_issue_notice'], 11);

        add_action('as3cf_licence_field', [$this, 'render_licence_settings']);
        add_action('as3cf_support_pre_debug', [$this, 'render_licence_info']);

        add_filter('as3cfpro_js_nonces', [$this, 'add_licence_nonces']);
        add_filter('as3cfpro_js_strings', [$this, 'add_licence_strings']);
        add_filter('as3cf_addons', [$this, 'inject_addon_page_links']);

        add_action('as3cf_plugin_load', [$this, 'http_dismiss_licence_notice']);
        add_action('as3cfpro_http_refresh_licence', [$this, 'do_http_refresh_licence']);
        add_action('as3cfpro_http_remove_licence', [$this, 'do_http_remove_licence']);
        add_action('as3cfpro_activate_licence_response', [$this, 'refresh_licence_notice']);
        add_action('as3cfpro_ajax_check_licence_response', [$this, 'refresh_licence_notice']);
        add_filter('as3cfpro_licence_status_message', [$this, 'licence_status_message'], 10, 2);
        add_filter('as3cfpro_pre_plugin_row_update_notice', [$this, 'suppress_plugin_row_update_notices'], 10, 2);
        add_action('check_admin_referer', [$this, 'block_updates_with_invalid_license']);
    }
     ...
     * @return array $addons
     */
    public function inject_addon_page_links($addons)
    {
     ...
     */
    public function do_http_remove_licence()
    {
        $this->remove_media_transients();
    }
     ...
     */
    public function remove_media_transients()
    {
    ...
    public function licence_issue_notice($dashboard = false, $skip_transient = false)
    {
        ...
        } elseif (! empty($license_check['errors']['subscription_expired'])) {
            $this->display_expired_licence_notice($args);
        } elseif ($media_limit_check['status']['code'] === self::MEDIA_USAGE_APPROACHING) {
    ...
    protected function display_no_licence_notice($args)
    {
        ...
        $this->render_licence_notice(array_merge($args, [
    ...
    protected function display_near_limit_licence_notice($args)
    {
        ...
        $args['links'] = ['upgrade_now', 'check_again'];
     ...
     * @param  array  $args
     */
    public function render_licence_notice($args = [])
    {
        $args = array_merge([
            'title' => '',
            'type' => '',
    ...
    {
        return $this->as3cf->dbrains_url('/wp-offload-media/doc/non-essential-features/', [
    ...
    {
        return $this->as3cf->dbrains_url('/wp-offload-media/pricing/', [
            'utm_campaign' => 'error+messages',
        ], 'free-up-limit');
    }
    ...
    {
        if (isset($_GET['page']) && $_GET['page'] === 'amazon-s3-and-cloudfront') {
            return;
        }
    ...
    public function check_licence_media_limit($skip_transient = false)
    {
        ...
        if ($skip_transient || $media_limit_check === false || isset($media_limit_check['errors'])) {
    ...
    public function licence_status_message($message, $errors)
    {
        if (isset($errors['subscription_expired'])) {
            ...
            $url = $this->as3cf->dbrains_url('/my-account/', [
                'utm_campaign' => 'error+messages',
            ]);
            ...
            if (in_array($plugin, $our_plugins)) {
                ...
                if (isset($_GET['plugins'])) {
                    ...
                    foreach ($_POST['checked'] as $key => $checked_plugin) {
                        if (in_array($checked_plugin, [$plugin, urlencode($plugin)])) {
                            // Remove plugin from the global var
                ...
                $this->as3cf->render_view('error-fatal', ['message' => $html]);
            }
        }
