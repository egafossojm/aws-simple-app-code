...
abstract class Modal_Tool extends Tool
{
     ...
     * @var array
     */
    protected static $total_attachments = [];

    /**
     * @var array
     */
    public static $views_rendered = [];
    ...
    {
        // JS data
        add_filter('as3cfpro_js_settings', [$this, 'add_js_settings']);
        add_filter('as3cfpro_js_strings', [$this, 'add_js_strings']);
        add_filter('as3cfpro_js_nonces', [$this, 'add_js_nonces']);

        // AJAX
        add_action('wp_ajax_as3cfpro_initiate_'.$this->tool_key, [$this, 'ajax_initiate_process']);
        add_action('wp_ajax_as3cfpro_calculate_items_'.$this->tool_key, [$this, 'ajax_calculate_items']);
        add_action('wp_ajax_as3cfpro_process_items_'.$this->tool_key, [$this, 'ajax_process_items']);
        add_action('wp_ajax_as3cfpro_finish_'.$this->tool_key, [$this, 'ajax_finish_process']);

        // Views
        add_action('as3cf_post_settings_render', [$this, 'render_modal']);
     ...
     * @return array
     */
    public function get_tool_js_settings($settings)
    {
        return $settings;
    }
    ...
    public function ajax_calculate_items()
    {
        ...
        if ((int) $this->progress['total_files'] === 0 && empty($this->progress['more_blogs'])) {
    ...
    public function load_assets()
    {
        if (! self::$assets_loaded) {
            $this->as3cf->enqueue_style('as3cf-pro-tool-styles', 'assets/css/pro/tool', ['as3cf-pro-styles']);
            $this->as3cf->enqueue_script('as3cf-pro-tool-script', 'assets/js/pro/tool', ['as3cf-pro-script', 'underscore']);
    ...
    protected function capture_error_html()
    {
        ...
        $this->as3cf->render_view('tool-errors', [
            'tool' => $this->tool_key,
