...
class Downloader extends Modal_Tool
{
    ...
    protected function get_sidebar_block_args()
    {
        ...
        if ($to_process_stats['total_attachments'] === 0) {
            return false;
        }

        // Don't show tool if no attachments uploaded to bucket
        if ($to_process_stats['total_to_process'] === 0) {
            return false;
        }
    ...
    protected function handle_attachment($attachment_id, $blog_id)
    {
        ...
        if (is_wp_error($result)) {
            ...
            if ($this->progress['error_count'] <= 100) {
                // Build error message
                $errors = is_array($result->get_error_data()) ? $result->get_error_data() : [];
    ...
    public function maybe_render_deactivate_prompt()
    {
        ...
        add_action('load-plugins.php', [$this, 'deactivate_plugin_assets']);
        add_action('admin_footer', [$this, 'deactivate_plugin_modal']);
    ...
    {
        $this->as3cf->enqueue_script('as3cf-pro-tool-downloader-script', 'assets/js/pro/tool-downloader', ['as3cf-modal']);

        wp_localize_script('as3cf-pro-tool-downloader-script', 'as3cfpro_downloader', [
            'plugin_url' => $this->as3cf->get_plugin_page_url(['tool' => $this->tool_key]),
