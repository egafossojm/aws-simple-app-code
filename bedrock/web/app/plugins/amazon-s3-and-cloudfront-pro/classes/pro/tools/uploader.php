...
class Uploader extends Modal_Tool
{
    ...
    protected function get_attachments_to_process($prefix, $blog_id, $count = false, $limit = null, $offset = null, $exclude = null)
    {
         ...
         * @param array
         */
        $ignored_mime_types = apply_filters('as3cf_ignored_mime_types', []);
    ...
    protected function get_ajax_initiate_data()
    {
        ...
        if ($total_allowed_items_to_upload === false) {
            $response = [
                'success' => false,
                'data' => __('Unable to reach deliciousbrains.com ', 'amazon-s3-and-cloudfront'),
                'errors' => [__('We have been unable to retrieve the number of items you can offload. Please try again later, or contact support if the issue persists.', 'amazon-s3-and-cloudfront')],
            ];
    ...
    protected function get_sidebar_block_args()
    {
        ...
        if ($human_percentage === 0 && $uploaded_percentage > 0) {
            $human_percentage = 1;
        }
    ...
    protected function should_upload_attachment($attachment_id, $blog_id)
    {
        ...
        if ($total_allowed === -1 || $total_allowed > 0) {
            return;
        }
    ...
    public function get_status()
    {
        ...
        return array_merge(parent::get_status(), [
