...
class Meta_Slider extends Integration
{
    ...
    {
        add_filter('metaslider_attachment_url', [$this, 'metaslider_attachment_url'], 10, 2);
        add_filter('sanitize_post_meta_amazonS3_info', [$this, 'layer_slide_sanitize_post_meta'], 10, 3);
        add_filter('as3cf_pre_update_attachment_metadata', [$this, 'layer_slide_abort_upload'], 10, 4);
        add_filter('as3cf_remove_attachment_paths', [$this, 'layer_slide_remove_attachment_paths'], 10, 4);
        add_action('shutdown', [$this, 'layer_slide_remove_local_files']);
    ...
    public function metaslider_attachment_url($url, $slide_id)
    {
        ...
        if (! is_wp_error($provider_url) && $provider_url !== false) {
            return $provider_url;
        }
    ...
    {
        if (filter_input(INPUT_POST, 'action') === 'create_html_overlay_slide') {
            return true;
        }
    ...
    private function maybe_upload_attachment_backup_sizes($object_id, $meta_key, $data)
    {
        ...
        if (filter_input(INPUT_POST, 'action') !== 'resize_image_slide' && ! $this->is_layer_slide()) {
            return;
        }
