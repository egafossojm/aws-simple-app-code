...
class Advanced_Custom_Fields extends Integration
{
    ...
    public function init()
    {
         ...
         * Content Filtering
         */
        add_filter('acf/load_value/type=text', [$this->as3cf->filter_local, 'filter_post']);
        add_filter('acf/load_value/type=textarea', [$this->as3cf->filter_local, 'filter_post']);
        add_filter('acf/load_value/type=wysiwyg', [$this->as3cf->filter_local, 'filter_post']);
        add_filter('acf/update_value/type=text', [$this->as3cf->filter_provider, 'filter_post']);
        add_filter('acf/update_value/type=textarea', [$this->as3cf->filter_provider, 'filter_post']);
        add_filter('acf/update_value/type=wysiwyg', [$this->as3cf->filter_provider, 'filter_post']);
        ...
        if (class_exists('acf_field_image_crop')) {
            add_filter('wp_get_attachment_metadata', [$this, 'download_image'], 10, 2);
            add_filter('sanitize_file_name', [$this, 'remove_original_after_download']);
        }
    }
    ...
    {
        if ($this->as3cf->plugin_compat->maybe_process_on_action('acf_image_crop_perform_crop', true) === false) {
        ...
        foreach ($callers as $caller) {
            if (isset($caller['function']) && $caller['function'] === 'image_downsize') {
    ...
    public function maybe_remove_original_after_download()
    {
        ...
        $this->as3cf->remove_local_files([$original_file]);
