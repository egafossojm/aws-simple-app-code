...
class Easy_Digital_Downloads extends Integration
{
    ...
    public function init()
    {
        ...
        add_filter('edd_file_download_method', [$this, 'set_download_method']);
        // Disable using symlinks for download.
        add_filter('edd_symlink_file_downloads', [$this, 'disable_symlink_file_downloads']);
        // Hook into edd_requested_file to swap in the S3 secure URL
        add_filter('edd_requested_file', [$this, 'get_download_url'], 10, 3);
        // Hook into the save download files metabox to apply the private ACL
        add_filter('edd_metabox_save_edd_download_files', [$this, 'make_edd_files_private_on_provider'], 11);
    }
    ...
    public function get_download_url($file, $download_files, $file_key)
    {
        ...
        $headers = apply_filters('as3cf_edd_download_headers', [
        ...
        if (($file_name[0] !== '/' && isset($url['scheme']) === false) || false !== (strpos($file_name, 'AWSAccessKeyId'))) {
     ...
     */
    public function maybe_make_removed_edd_files_public($attachment_ids, $download_id)
    {
        global $wpdb;
