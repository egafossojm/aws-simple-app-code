...
class Enable_Media_Replace extends Integration
{
    ...
    {
        add_filter('as3cf_get_attached_file', [$this, 'download_file'], 10, 4);
        add_filter('update_attached_file', [$this, 'maybe_process_provider_replacement'], 101, 2);
        add_filter('as3cf_update_attached_file', [$this, 'process_provider_replacement'], 10, 2);
        add_filter('as3cf_get_attachment_provider_info', [$this, 'update_file_prefix_on_replace'], 10, 2);
        add_filter('as3cf_pre_update_attachment_metadata', [$this, 'remove_existing_provider_files_before_replace'], 10, 4);
        add_filter('emr_unfiltered_get_attached_file', '__return_false');
        add_filter('emr_unique_filename', [$this, 'ensure_unique_filename'], 10, 3);
    }
     ...
     * @return string
     */
    public function download_file($url, $file, $attachment_id, $provider_object)
    {
     ...
     * @return bool
     */
    public function remove_existing_provider_files_before_replace($pre, $data, $post_id, $provider_object = [])
    {
     ...
     * @return string
     */
    public function process_provider_replacement($file, $attachment_id)
    {
        if (! $this->is_replacing_media()) {
            return $file;
        }
    ...
    public function is_replacing_media()
    {
        ...
        return  sanitize_key($action) === 'media_replace_upload';
    }
    ...
    public function update_file_prefix_on_replace($provider_object, $attachment_id)
    {
        ...
        foreach ($callers as $caller) {
            if (isset($caller['function']) && $caller['function'] === 'upload_attachment') {
