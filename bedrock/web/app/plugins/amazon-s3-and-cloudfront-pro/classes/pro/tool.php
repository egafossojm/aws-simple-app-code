...
abstract class Tool
{
    ...
    {
        // Load sidebar block
        add_action('as3cfpro_sidebar', [$this, 'render_sidebar_block'], $this->priority);

        // Ajax notices
        add_action('wp_ajax_as3cfpro_dismiss_errors_'.$this->tool_key, [$this, 'ajax_dismiss_errors']);
    }
     ...
     * @return array
     */
    public function get_errors($default = [])
    {
     ...
     * @param  array  $errors
     */
    public function update_error_notice($errors = [])
    {
        if (empty($errors)) {
        ...
        if (! empty($errors)) {
            $args = [
                ...
                'user_capabilities' => ['as3cfpro', 'is_plugin_setup'],
                'show_callback' => ['as3cfpro', 'render_tool_errors_callback'],
                'callback_args' => [$this->tool_key],
            ];
