...
class AS3CF_Notices
{
    ...
    protected function __construct($as3cf)
    {
        ...
        add_action('as3cf_pre_tab_render', [$this, 'admin_notices']);
        add_action('admin_enqueue_scripts', [$this, 'enqueue_notice_scripts']);
        add_action('wp_ajax_as3cf-dismiss-notice', [$this, 'ajax_dismiss_notice']);
    }
     ...
     */
    public function add_notice($message, $args = [])
    {
        $defaults = [
            'type' => 'info',
            'dismissible' => true,
            ...
            'user_capabilities' => ['as3cf_compat_check', 'check_capabilities'], // A user with these capabilities can see the notice. Can be a callback with the first array item the name of global class instance.
            ...
            'callback_args' => [], // Arguments to pass to the callback.
        ...
        if (isset($notice['only_show_on_tab']) && $notice['only_show_on_tab'] !== false) {
            $notice['inline'] = true;
        }
    ...
    protected function maybe_show_notice($notice, $dismissed_notices, $tab)
    {
        ...
        if ($notice['only_show_in_settings'] && strpos($screen->id, $this->as3cf->hook_suffix) === false) {
            return;
        }
        ...
        if (! isset($notice['only_show_on_tab']) && $tab !== false) {
            return;
        }
    ...
    protected function check_capability_for_notice($notice)
    {
        ...
        if (count($caps) === 2 && isset($GLOBALS[$caps[0]]) && is_callable([$GLOBALS[$caps[0]], $caps[1]])) {
            // Handle callback passed for capabilities
            return call_user_func([$GLOBALS[$caps[0]], $caps[1]]);
        }
    ...
    public function enqueue_notice_scripts()
    {
        ...
        $this->as3cf->enqueue_script('as3cf-notice', 'assets/js/notice', ['jquery']);

        wp_localize_script('as3cf-notice', 'as3cf_notice', [
            'strings' => [
    ...
    public function ajax_dismiss_notice()
    {
        ...
        if (! isset($_POST['notice_id']) || ! ($notice_id = sanitize_text_field($_POST['notice_id']))) {
            $out = ['error' => __('Invalid notice ID.', 'amazon-s3-and-cloudfront')];
