...
class AS3CF_Plugin_Compatibility
{
     ...
     * @var array
     */
    protected static $stream_wrappers = [];
    ...
    public function compatibility_init()
    {
        ...
        add_action('admin_init', [$this, 'maybe_warn_about_php_version']);
     ...
     */
    public function compatibility_init_if_setup()
    {
        // Turn on stream wrapper S3 file
        add_filter('as3cf_get_attached_file', [$this, 'get_stream_wrapper_file'], 20, 4);

        /*
         * Legacy filter
         * 'as3cf_get_attached_file_copy_back_to_local'
         */
        add_filter('as3cf_get_attached_file', [$this, 'legacy_copy_back_to_local'], 10, 4);

        /*
         * WP_Image_Editor
         * /wp-includes/class-wp-image-editor.php
         */
        add_action('as3cf_pre_upload_attachment', [$this, 'image_editor_remove_files'], 10, 3);
        add_filter('as3cf_get_attached_file', [$this, 'image_editor_download_file'], 10, 4);
        add_filter('as3cf_upload_attachment_local_files_to_remove', [$this, 'image_editor_remove_original_image'], 10, 3);
        add_filter('as3cf_get_attached_file', [$this, 'customizer_crop_download_file'], 10, 4);
        add_filter('as3cf_upload_attachment_local_files_to_remove', [$this, 'customizer_crop_remove_original_image'], 10, 3);
        add_filter('wp_unique_filename', [$this, 'customizer_crop_unique_filename'], 10, 3);

        /*
         * Regenerate Thumbnails (before v3)
         * https://wordpress.org/plugins/regenerate-thumbnails/
         */
        add_filter('as3cf_get_attached_file', [$this, 'regenerate_thumbnails_download_file'], 10, 4);
        ...
        if (defined('WP_CLI') && class_exists('WP_CLI')) {
            WP_CLI::add_hook('before_invoke:media regenerate', [$this, 'enable_get_attached_file_copy_back_to_local']);
        }
    }
    ...
    public function enable_get_attached_file_copy_back_to_local()
    {
        ...
        add_filter('as3cf_upload_attachment_local_files_to_remove', [
            $this,
            'monitor_local_files_to_remove',
        ], 10, 3);

        // Prevent subsequent attempts to copy back after upload and remove.
        add_filter('as3cf_get_attached_file_copy_back_to_local', [
            $this,
     ...
     * @return string
     */
    public function copy_image_to_server_on_action($action_key, $ajax, $url, $file, $provider_object)
    {
        if ($this->maybe_process_on_action($action_key, $ajax) === false) {
            return $url;
        }
     ...
     * @return bool|string
     */
    public function get_original_image_file($post_id, $file_path)
    {
     ...
     * @return string
     */
    public function image_editor_download_file($url, $file, $attachment_id, $provider_object)
    {
        if (! $this->is_ajax()) {
            return $url;
        }
        ...
        if (in_array($action, ['image-editor', 'imgedit-preview'])) { // input var okay
            foreach (debug_backtrace() as $caller) {
                if (isset($caller['function']) && $caller['function'] == '_load_image_to_edit_path') {
     ...
     * @return array
     */
    public function image_editor_remove_original_image($files, $post_id, $file_path)
    {
        if (! $this->is_ajax()) {
            return $files;
        }

        if (isset($_POST['action']) && sanitize_key($_POST['action']) === 'image-editor') { // input var okay
            // remove original main image after edit
    ...
    {
        if ($this->is_customizer_crop_action() === false) {
            return $url;
        }
     ...
     * @return array
     */
    public function customizer_crop_remove_original_image($files, $post_id, $file_path)
    {
        if ($this->is_customizer_crop_action() === false) {
            return $files;
        }
     ...
     * @return string
     */
    public function regenerate_thumbnails_download_file($url, $file, $attachment_id, $provider_object)
    {
    ...
    public function copy_provider_file_to_server($provider_object, $file)
    {
        ...
        try {
            $this->as3cf->get_provider_client($provider_object['region'], true)->get_object([
                'Bucket' => $provider_object['bucket'],
    ...
    public function wp_calculate_image_srcset_meta($image_meta, $size_array, $image_src, $attachment_id)
    {
        ...
        if (strpos($provider_object['key'], $image_basename) === false) {
    ...
    public function wp_calculate_image_srcset($sources, $size_array, $image_src, $image_meta, $attachment_id)
    {
        ...
        foreach ($sources as $width => $source) {
            ...
            if ($provider_url === false || is_wp_error($provider_url)) {
