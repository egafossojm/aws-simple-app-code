...
class Amazon_S3_And_CloudFront extends AS3CF_Plugin_Base
{
     ...
     */
    protected static $provider_classes = [];
     ...
     * @var array
     */
    protected static $settings_constants = [
        'AS3CF_SETTINGS',
        'WPOS3_SETTINGS',
    ];
    ...
    public function init($plugin_file_path)
    {
        ...
        add_filter('pre_get_space_used', [$this, 'multisite_get_space_used']);
        ...
        add_action('as3cf_plugin_load', [$this, 'remove_access_keys_if_constants_set']);

        // Attachment screens/modals
        add_action('load-upload.php', [$this, 'load_media_assets'], 11);
        add_action('admin_enqueue_scripts', [$this, 'load_attachment_assets'], 11);
        add_action('add_meta_boxes', [$this, 'attachment_provider_meta_box']);

        // UI AJAX
        add_action('wp_ajax_as3cf-get-buckets', [$this, 'ajax_get_buckets']);
        add_action('wp_ajax_as3cf-save-bucket', [$this, 'ajax_save_bucket']);
        add_action('wp_ajax_as3cf-create-bucket', [$this, 'ajax_create_bucket']);
        add_action('wp_ajax_as3cf-manual-save-bucket', [$this, 'ajax_save_bucket']);
        add_action('wp_ajax_as3cf-get-url-preview', [$this, 'ajax_get_url_preview']);
        add_action('wp_ajax_as3cf_get_attachment_provider_details', [$this, 'ajax_get_attachment_provider_details']);
        add_action('wp_ajax_as3cf-get-diagnostic-info', [$this, 'ajax_get_diagnostic_info']);

        // Rewriting URLs, doesn't depend on plugin being setup
        add_filter('wp_get_attachment_url', [$this, 'wp_get_attachment_url'], 99, 2);
        add_filter('get_image_tag', [$this, 'maybe_encode_get_image_tag'], 99, 6);
        add_filter('wp_get_attachment_image_src', [$this, 'maybe_encode_wp_get_attachment_image_src'], 99, 4);
        add_filter('wp_prepare_attachment_for_js', [$this, 'maybe_encode_wp_prepare_attachment_for_js'], 99, 3);
        add_filter('image_get_intermediate_size', [$this, 'maybe_encode_image_get_intermediate_size'], 99, 3);
        ...
        add_filter('wp_handle_upload_prefilter', [$this, 'wp_handle_upload_prefilter'], 1);
        add_filter('wp_handle_sideload_prefilter', [$this, 'wp_handle_upload_prefilter'], 1);
        add_filter('wp_update_attachment_metadata', [$this, 'wp_update_attachment_metadata'], 110, 2);
        ...
        if (static::settings_constant() !== false) {
            add_action('as3cf_constant_'.static::settings_constant().'_changed_bucket', [$this, 'bucket_changed']);
        }
    ...
    public function get_setting($key, $default = '')
    {
        ...
        if (isset($_POST['action']) && sanitize_key($_POST['action']) == 'as3cf-get-url-preview') { // input var okay
        ...
        if ($key === 'post_meta_version' && ! isset($settings['post_meta_version'])) {
        ...
        if ($key == 'object-versioning' && ! isset($settings['object-versioning'])) {
            return '1';
        }

        // Default object prefix
        if ($key == 'object-prefix' && ! isset($settings['object-prefix'])) {
            return $this->get_default_object_prefix();
        }

        // Default use year and month folders
        if ($key == 'use-yearmonth-folders' && ! isset($settings['use-yearmonth-folders'])) {
            return get_option('uploads_use_yearmonth_folders');
        }

        // Default enable object prefix - enabled unless path is empty
        if ($key == 'enable-object-prefix') {
            if (isset($settings['enable-object-prefix']) && $settings['enable-object-prefix'] == '0') {
                return 0;
            }

            if (isset($settings['object-prefix']) && trim($settings['object-prefix']) == '') {
                if ($this->get_defined_setting('object-prefix', false) === false) {
                    return 0;
                }
            } else {
                return 1;
        ...
        if ($key === 'domain' && ! isset($settings['domain'])) {
        ...
        if ($key === 'domain' && $settings[$key] === 'virtual-host') {
            return $this->upgrade_virtual_host();
        }

        // SSL radio buttons since 0.8
        if ($key == 'ssl' && ! isset($settings['ssl'])) {
        ...
        if ($key === 'force-https' && ! isset($settings['force-https'])) {
        ...
        if ($key === 'access-key-id' && ! isset($settings['access-key-id'])) {
        ...
        if ($key === 'secret-access-key' && ! isset($settings['secret-access-key'])) {
    ...
    {
        if ($key === 'provider' && defined($constant)) {
    ...
    public function get_setting_region($settings, $key, $default)
    {
        ...
        if ($key === 'region' && ! isset($settings['region'])) {
    ...
    {
        if ($key === 'bucket' && defined($constant)) {
     ...
     * @return string
     */
    public function maybe_disable_save_button($defined_settings = [])
    {
        ...
        foreach ($whitelisted_settings as $setting) {
            ...
            if ($setting === 'object-prefix') {
                if (isset($defined_settings['enable-object-prefix']) && $defined_settings['enable-object-prefix'] === '0') {
                    continue;
                }
            }

            if ($setting === 'cloudfront') {
                if (isset($defined_settings['domain']) && $defined_settings['domain'] !== 'cloudfront') {
                    continue;
                }
            }
     ...
     * @param  array  $args
     */
    public function schedule_event($hook, $interval = null, $args = [])
    {
        if (is_null($interval)) {
            $interval = $hook;
        }
    ...
    public function get_url_preview($escape = true, $suffix = 'photo.jpg')
    {
        ...
        $domain = $this->get_provider()->get_url_domain($bucket, $region, null, [], true);
     ...
     */
    public function ajax_get_url_preview()
    {
        $this->verify_ajax_request();
     ...
     * @return bool
     */
    public function delete_objects($region, $bucket, $objects, $log_error = false, $return_on_error = false, $force_new_provider_client = false)
    {
        $chunks = array_chunk($objects, 1000);

        try {
            foreach ($chunks as $chunk) {
                $this->get_provider_client($region, $force_new_provider_client)->delete_objects([
                    'Bucket' => $bucket,
                    'Objects' => $chunk,
                ]);
            }
     ...
     *                                           to cope with possible different regions
     */
    public function remove_attachment_files_from_provider($post_id, $provider_object, $remove_backup_sizes = true, $log_error = false, $return_on_error = false, $force_new_provider_client = false)
    {
     ...
     */
    public function remove_local_files($file_paths, $attachment_id = 0)
    {
        $filesize_total = 0;
        ...
        foreach ($file_paths as $index => $path) {
            if (! empty($attachment_id) && is_int($attachment_id)) {
                ...
                $filesize_total += ($bytes !== false) ? $bytes : 0;
            }
     ...
     * @return string
     */
    public function get_object_version_string()
    {
     ...
     * @return null|string
     */
    public function get_folder_time_from_url($url)
    {
        if (! is_string($url)) {
            return null;
        }
     ...
     * @return int|string
     */
    public function get_attachment_folder_time($post_id)
    {
        $time = current_time('timestamp');
     ...
     * @return bool
     */
    public function does_file_exist($filename, $time)
    {
     ...
     * @throws Exception
     */
    public function does_file_exist_provider($filename, $time)
    {
        $bucket = $this->get_setting('bucket');
        $region = $this->get_setting('region');
     ...
     * @return string
     */
    public function generate_unique_filename($name, $ext, $time)
    {
        $count = 1;
    ...
    public function is_plugin_setup($with_credentials = false)
    {
        ...
        if ((bool) $this->get_setting('bucket') === false) {
            // No bucket selected
            return false;
        }
     ...
     */
    public function get_secure_attachment_url($post_id, $expires = null, $size = null, $headers = [], $skip_rewrite_check = false)
    {
        if (is_null($expires)) {
     ...
     * @return string
     */
    public function get_object_prefix($toggle_setting = 'enable-object-prefix')
    {
     ...
     */
    public function get_attachment_url($post_id, $expires = null, $size = null, $meta = null, $headers = [], $skip_rewrite_check = false)
    {
    ...
    public function get_attachment_local_url($post_id)
    {
        ...
        if ($file = get_post_meta($post_id, '_wp_attached_file', true)) {
            ...
            if (($uploads = wp_upload_dir()) && $uploads['error'] === false) {
     ...
     * @return mixed|WP_Error
     */
    public function get_attachment_provider_url($post_id, $provider_object, $expires = null, $size = null, $meta = null, $headers = [])
    {
        $scheme = $this->get_url_scheme();
     ...
     * @param  array  $return
     */
    public function end_ajax($return = [])
    {
        wp_send_json($return);
    }
     ...
     * @return bool
     */
    public function verify_ajax_request($capability = null, $return = false)
    {
     ...
     * @return string
     */
    public function ajax_check_region($region_required = false)
    {
        $region = '';
        ...
        if (defined('AS3CF_REGION')) {
            $region = AS3CF_REGION;
        } elseif ($this->get_defined_setting('region', false) !== false) {
            ...
            $out = ['error' => __('No region provided.', 'amazon-s3-and-cloudfront')];
     ...
     * @return string
     */
    public function ajax_check_bucket()
    {
        if (! isset($_POST['bucket_name']) || ! ($bucket = sanitize_text_field($_POST['bucket_name']))) { // input var okay
            $out = ['error' => __('No bucket name provided.', 'amazon-s3-and-cloudfront')];
     ...
     */
    public function create_bucket($bucket_name, $region = false)
    {
        try {
     ...
     * @throws Exception
     */
    public function ajax_save_bucket()
    {
        $this->verify_ajax_request();
        ...
        if (isset($_POST['action']) && strpos($_POST['action'], 'manual-save-bucket') !== false) {
            $manual = true;
        }
     ...
     * @throws Exception
     */
    public function save_bucket_for_ajax($bucket, $manual_select = false, $region = null, $defaults = [])
    {
     ...
     * @return array
     */
    public function prepare_bucket_error($object, $single = true)
    {
        if ($object->get_error_message() === 'Access Denied') {
            // If the bucket error is access denied, show our notice message
            $out = ['error' => $this->get_access_denied_notice_message($single)];
        } else {
            $out = ['error' => $object->get_error_message()];
        }
     ...
     * @param  string  $value
     */
    public function pre_set_setting($key, $value)
    {
        if ($key === 'bucket' && ! $this->get_setting('bucket')) {
     ...
     */
    public function save_bucket($bucket_name, $manual = false, $region = null)
    {
        if ($bucket_name) {
            $this->get_settings();
    ...
    public function get_provider_client($region = false, $force = false)
    {
        if (is_null($this->provider_client) ||
             ...
             $force ||
             ($region !== false && $this->provider_client_region !== $region)) {
            $args = [];
    ...
    public function get_bucket_region($bucket, $use_cache = false)
    {
        ...
        try {
            $region = $this->get_provider_client(false, true)->get_bucket_location(['Bucket' => $bucket]);
        } catch (Exception $e) {
     ...
     */
    public function get_provider_object_region($provider_object, $post_id = null)
    {
     ...
     */
    public function get_buckets($region = false)
    {
        try {
     ...
     * @throws Exception
     */
    public function check_write_permission($bucket = null, $region = null)
    {
     ...
     */
    public function render_bucket_permission_errors()
    {
        $can_write = $this->check_write_permission();
        // catch any checking issues
        if (is_wp_error($can_write)) {
            $this->render_view('error-fatal', ['message' => $can_write->get_error_message()]);
            $can_write = true;
        }
        // display a error message if the user does not have write permission to S3 bucket
        $this->render_view('error-access', ['can_write' => $can_write]);
    }
    ...
    public function plugin_load()
    {
        ...
        add_action('network_admin_notices', [$this, 'settings_saved_notice']);

        $this->enqueue_style('as3cf-styles', 'assets/css/styles', ['as3cf-modal', 'as3cf-storage-provider']);
        $this->enqueue_script('as3cf-script', 'assets/js/script', ['jquery', 'underscore', 'as3cf-modal', 'as3cf-storage-provider']);
     ...
     * @return array
     */
    public function get_skip_sanitize_settings()
    {
        return [];
    }
    ...
    public function handle_post_request()
    {
        ...
        if (empty($_POST['action']) || sanitize_key($_POST['action']) != 'save') { // input var okay
            return;
        }

        if (empty($_POST['_wpnonce']) || ! wp_verify_nonce(sanitize_key($_POST['_wpnonce']), $this->get_settings_nonce_key())) { // input var okay
            exit(__("Cheatin' eh?", 'amazon-s3-and-cloudfront'));
        }
        ...
        foreach ($post_vars as $var) {
            ...
            if ($var === 'secret-access-key' && ! empty($_POST['secret-access-key']) && _x('-- not shown --', 'placeholder for hidden secret access key, 39 char max', 'amazon-s3-and-cloudfront') === $_POST['secret-access-key']) {
                continue;
            }
        ...
        $url = $this->get_plugin_page_url(['updated' => '1']);
        wp_redirect($url);
        exit;
    }
    ...
    {
        $this->render_view('header', ['page_title' => $this->get_plugin_page_title(), 'page' => 'as3cf']);
     ...
     */
    public function render_addons($addons = null)
    {
        if (is_null($addons)) {
    ...
    public function get_dynamic_prefix($time = null)
    {
        ...
        if (is_multisite() && ! (is_main_network() && is_main_site()) && strpos($prefix, 'sites/') === false) {
     ...
     * @return string
     */
    public function get_year_month_directory_name($time = null)
    {
     ...
     * @param  array  $provider_object
     */
    public function make_acl_admin_notice($provider_object)
    {
     ...
     */
    public function check_for_gd_imagick()
    {
        if (! $this->is_plugin_setup(true)) {
        ...
        if (! $gd_enabled && ! $imagick_enabled) {
            $this->notices->add_notice(
                ...
                ['flash' => false, 'only_show_to_user' => false, 'only_show_in_settings' => true]
            );
        }
    }
     ...
     * @return string
     */
    public function get_image_sizes_details()
    {
     ...
     * @return string
     */
    public function get_acl_display_name($acl)
    {
        $acl = ($acl === 'public-read') ? 'public' : $acl;
     ...
     * @return bool
     */
    public function imagick_enabled()
    {
     ...
     */
    public function get_all_blog_table_prefixes($exclude_blog_ids = [])
    {
        global $wpdb;
        $prefix = $wpdb->prefix;
     ...
     * @return string
     */
    public function get_access_denied_notice_message($single = true)
    {
        if ($this->get_provider()->needs_access_keys()) {
        ...
        $url = $this->dbrains_url('/wp-offload-media/doc/quick-start-guide/', [
     ...
     * @return float|int
     */
    public function multisite_get_space_used($space_used)
    {
        global $wpdb;
    ...
    public function plugin_deactivated_notice()
    {
        if (false !== ($deactivated_notice_id = get_transient('as3cf_deactivated_notice_id'))) {
            ...
            $this->render_view('notice', ['message' => $message]);
     ...
     * @return mixed
     */
    public function filter_input($variable, $type = INPUT_GET, $filter = FILTER_DEFAULT, $options = [])
    {
    ...
    public function show_deprecated_domain_setting($domain = null)
    {
        ...
        if (! in_array($domain, ['path', 'cloudfront'])) {
            return true;
        }
    ...
    {
        if ($this->get_setting('ssl', 'request') !== 'http' || ! $this->is_plugin_setup()) {
            return;
        }
        ...
        $doc_url = $this->dbrains_url('/wp-offload-media/doc/force-http-setting/', [
            'utm_campaign' => 'support+docs',
        ]);
    ...
    {
        if ($this->get_setting('domain') === 'cloudfront') {
    ...
    public function attachment_provider_meta_box()
    {
        add_meta_box(
            ...
            [$this, 'attachment_provider_actions_meta_box'],
            'attachment',
            'side',
            'core'
        );
    ...
    {
        $strings = apply_filters('as3cf_media_action_strings', [
    ...
    {
        $this->enqueue_style('as3cf-media-styles', 'assets/css/media', ['as3cf-modal']);
        $this->enqueue_script('as3cf-media-script', 'assets/js/media', [
            'jquery',
            'media-views',
            'media-grid',
            'wp-util',
        ...
        wp_localize_script('as3cf-media-script', 'as3cf_media', [
    ...
    public function ajax_get_attachment_provider_details()
    {
        ...
        $data = [
            'links' => $this->add_media_row_actions([], $id),
    ...
    {
        global $post;
        if ($hook_suffix !== 'post.php' || $post->post_type !== 'attachment') {
            return;
        }

        $this->enqueue_style('as3cf-pro-attachment-styles', 'assets/css/attachment', ['as3cf-modal']);
    ...
    public function maybe_fix_local_subsite_url($url)
    {
        ...
        if (is_multisite() && ! $this->is_current_blog(get_current_blog_id()) && strpos($url, $siteurl) !== 0) {
    ...
    protected function get_available_addons()
    {
        return [
            'amazon-s3-and-cloudfront-assets-pull' => [
                ...
                'url' => $this->dbrains_url('/wp-offload-media/doc/assets-pull-addon/', [
     ...
     * @return string
     */
    public function get_addon_icon_url($slug)
    {
    ...
    {
        add_action('aws_access_key_form_header', [$this, 'handle_aws_access_key_form_header']);
        ...
        if (is_plugin_active('amazon-web-services/amazon-web-services.php')) {
            ...
            if (is_a($this->get_provider(), '\DeliciousBrains\WP_Offload_Media\Providers\AWS_Provider') && $this->get_provider()->needs_access_keys()) {
                ...
                if (! empty($aws_settings['access_key_id']) && ! empty($aws_settings['secret_access_key']) && $this->get_setting('bucket') !== false) {
    ...
    public function handle_aws_access_key_form_header()
    {
        $notice['message'] = sprintf(
            ...
            $this->get_plugin_page_url(['hash' => 'settings']),
    ...
    public function maybe_cleanup_filesize_metadata($post_id, $data, $update_metadata = true)
    {
        ...
        if (
            ...
            strpos($data['mime_type'], 'image/') === 0 ||
            ! (strpos($data['mime_type'], 'audio/') === 0 || strpos($data['mime_type'], 'video/') === 0)
        ) {
            unset($data['filesize']);
        }
