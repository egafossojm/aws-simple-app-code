...
function get_rocket_option($option, $default = false)
{
    ...
    if ($option === 'consumer_key' && defined('WP_ROCKET_KEY')) {
        return WP_ROCKET_KEY;
    } elseif ($option === 'consumer_email' && defined('WP_ROCKET_EMAIL')) {
...
{
    $cdn_cnames = get_rocket_cdn_cnames(['all', 'images', 'css_and_js', 'css', 'js']);
...
{
    $uri = get_rocket_option('cache_reject_uri', []);
...
{
    if ($extension === 'css') {
        $excluded_files = get_rocket_option('exclude_css', []);
        /**
...
function get_rocket_exclude_async_css()
{
     ...
     */
    $exclude_async_css = apply_filters('rocket_exclude_async_css', []);
...
function rocket_valid_key()
{
    ...
    return strlen(get_rocket_option('consumer_key')) === 8 && hash_equals($rocket_secret_key, hash('crc32', get_rocket_option('consumer_email')));
}
