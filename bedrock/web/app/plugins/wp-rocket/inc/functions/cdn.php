 ...
 */
function get_rocket_cnames_host($zones = ['all'])
{
    $hosts = [];
 ...
 */
function get_rocket_cdn_url($url, $zone = ['all'])
{
...
function rocket_add_cdn_on_custom_attr($html)
{
    if (preg_match('/(data-lazy-src|data-lazyload|data-src|data-retina)=[\'"]?([^\'"\s>]+)[\'"]/i', $html, $matches)) {
        $html = str_replace($matches[2], get_rocket_cdn_url($matches[2], ['all', 'images']), $html);
    }
