...
function rocket_lazyload_script()
{
	...
	s.src = "'.get_rocket_cdn_url(WP_ROCKET_FRONT_JS_URL, ['all', 'css_and_js', 'js']).'lazyload-" + v + "'.$suffix.'.js";
	w.lazyLoadOptions = {
...
function rocket_lazyload_iframes($html)
{
    ...
    foreach ($matches as $iframe) {
        ...
        if (get_rocket_option('lazyload_youtube') && strpos($iframe[1], 'youtube') !== false) {
