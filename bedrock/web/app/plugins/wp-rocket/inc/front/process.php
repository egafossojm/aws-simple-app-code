...
$continue = false;
if (realpath($rocket_config_path.$host.'.php') && stripos(realpath($rocket_config_path.$host.'.php'), $real_rocket_config_path) === 0) {
    ...
    foreach ($path as $p) {
        ...
        if (realpath($rocket_config_path.$host.'.'.$p.'.php') && stripos(realpath($rocket_config_path.$host.'.'.$p.'.php'), $real_rocket_config_path) === 0) {
            include $rocket_config_path.$host.'.'.$p.'.php';
            $continue = true;
            break;
        }

        if (realpath($rocket_config_path.$host.'.'.$dir.$p.'.php') && stripos(realpath($rocket_config_path.$host.'.'.$dir.$p.'.php'), $real_rocket_config_path) === 0) {
...
function rocket_serve_cache_file($rocket_cache_filepath)
{
    ...
    if (isset($_SERVER['HTTP_ACCEPT_ENCODING']) && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip') !== false && file_exists($rocket_cache_filepath_gzip) && is_readable($rocket_cache_filepath_gzip)) {
