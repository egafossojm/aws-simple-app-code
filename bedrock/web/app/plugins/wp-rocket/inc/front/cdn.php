...
function rocket_cdn_file($url)
{
    ...
    if ($filter === 'template_directory_uri' && ! empty($rejected_files)) {
        return $url;
    }
...
function rocket_cdn_custom_files($html)
{
    ...
    if ($cnames) {
         ...
         */
        $filetypes = apply_filters('rocket_cdn_custom_filetypes', ['mp3', 'ogg', 'mp4', 'm4v', 'avi', 'mov', 'flv', 'swf', 'webm', 'pdf', 'doc', 'docx', 'txt', 'zip', 'tar', 'bz2', 'tgz', 'rar', 'jpg', 'jpeg', 'jpe', 'png', 'gif', 'webp', 'bmp', 'tiff']);
...
function rocket_cdn_enqueue($src)
{
    ...
    if (is_admin() || is_preview() || in_array($GLOBALS['pagenow'], ['wp-login.php', 'wp-register.php'], true) || defined('DONOTROCKETOPTIMIZE') && DONOTROCKETOPTIMIZE) {
        return $src;
    }
