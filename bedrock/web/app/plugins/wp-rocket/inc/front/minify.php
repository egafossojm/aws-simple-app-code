...
function rocket_minify_process($buffer)
{
    ...
    if ($enable_css || $enable_js || $enable_google_fonts) {
        [$buffer, $conditionals] = rocket_extract_ie_conditionals($buffer);
    }
...
{
    if (is_ssl() && strpos($url, 'https://') === false && ! in_array(rocket_extract_url_component($url, PHP_URL_HOST), get_rocket_cnames_host(['all', 'css_js', 'css', 'js']), true)) {
