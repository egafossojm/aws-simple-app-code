...
function rocket_browser_cache_busting($src)
{
    ...
    if ($current_filter === 'style_loader_src' && get_rocket_option('minify_css') && (! defined('DONOTMINIFYCSS') || ! DONOTMINIFYCSS) && ! is_rocket_post_excluded_option('minify_css')) {
        return $src;
    }

    if ($current_filter === 'script_loader_src' && get_rocket_option('minify_js') && (! defined('DONOTMINIFYJS') || ! DONOTMINIFYJS) && ! is_rocket_post_excluded_option('minify_js')) {
        return $src;
    }
...
function get_rocket_browser_cache_busting($src, $current_filter = '')
{
    ...
    if (strpos($src, 'ver='.$GLOBALS['wp_version']) !== false) {
        $src = rtrim(str_replace(['ver='.$GLOBALS['wp_version'], '?&', '&&'], ['', '?', '&'], $src), '?&');
    }
...
function rocket_cache_dynamic_resource($src)
{
     ...
     * @param  array  $excluded_files  An array of filepath to exclude.
     */
    $excluded_files = apply_filters('rocket_exclude_static_dynamic_resources', []);
