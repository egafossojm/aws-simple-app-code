    ...
    foreach ($full as $args) {
        ...
        switch ($args['type']) {
            ...
            case 'text':
                ...
                $autocomplete = in_array($args['name'], ['consumer_key', 'consumer_email'], true) ? ' autocomplete="off"' : '';
                $disabled = ($args['name'] === 'consumer_key' && defined('WP_ROCKET_KEY')) || ($args['name'] === 'consumer_email' && defined('WP_ROCKET_EMAIL')) ? ' disabled="disabled"' : $readonly;
                ?>
            ...
            case 'cloudflare_api_key':
                ...
                if ($args['name'] === 'cloudflare_api_key' && defined('WP_ROCKET_CF_API_KEY')) {
                    $value = WP_ROCKET_CF_API_KEY;
                }

                $value = esc_attr($value);
                $disabled = ($args['name'] === 'cloudflare_api_key' && defined('WP_ROCKET_CF_API_KEY')) ? ' disabled="disabled"' : $readonly;
...
function rocket_settings_callback($inputs)
{
    ...
    if ($inputs['purge_cron_interval'] < 10 && $inputs['purge_cron_unit'] === 'MINUTE_IN_SECONDS') {
    ...
    // Option: Critical CSS.
    $inputs['critical_css'] = ! empty($inputs['critical_css']) ? str_replace(['<style>', '</style>'], '', wp_kses($inputs['critical_css'], ["\'", '\"'])) : '';
    ...
    if ($inputs['schedule_automatic_cleanup'] !== 1 && ($inputs['automatic_cleanup_frequency'] !== 'daily' || $inputs['automatic_cleanup_frequency'] !== 'weekly' || $inputs['automatic_cleanup_frequency'] !== 'monthly')) {
     ...
     * Option : CDN
     */
    $inputs['cdn_cnames'] = isset($inputs['cdn_cnames']) ? array_unique(array_filter($inputs['cdn_cnames'])) : [];
    ...
    if (isset($_FILES['import']) && $_FILES['import']['size'] !== 0 && $settings = rocket_handle_settings_import($_FILES['import'], $filename_prefix, $inputs)) {
        $inputs = $settings;
    }
...
function rocket_after_save_options($oldvalue, $value)
{
    ...
    if (! empty($_POST) && isset($oldvalue['cloudflare_auto_settings'], $value['cloudflare_auto_settings']) && (int) $oldvalue['cloudflare_auto_settings'] !== (int) $value['cloudflare_auto_settings']) {
        ...
        if (is_wp_error($cf_cache_level_return)) {
            ...
            if ($cf_cache_level_return === 'aggressive') {
    ...
    if (isset($oldvalue['analytics_enabled'], $value['analytics_enabled']) && $oldvalue['analytics_enabled'] !== $value['analytics_enabled'] && (int) $value['analytics_enabled'] === 1) {
    ...
    if (! empty($_POST) && $oldvalue['wl_plugin_name'] !== $value['wl_plugin_name'] &&
        isset($_POST['option_page'], $_POST['action']) && $_POST['option_page'] === 'wp_rocket' && $_POST['action'] === 'update') {
        add_settings_error('general', 'settings_updated', __('Settings saved.', 'rocket'), 'updated');
...
{
    if (rocket_extract_url_component($value, PHP_URL_SCHEME) === 'https') {
...
function rocket_pre_main_option($newvalue, $oldvalue)
{
    ...
    if (isset($newvalue['cloudflare_domain'], $oldvalue['cloudflare_domain']) && $newvalue['cloudflare_domain'] !== $oldvalue['cloudflare_domain'] && (int) get_rocket_option('do_cloudflare') > 0 && phpversion() >= '5.4') {
        require WP_ROCKET_ADMIN_PATH.'compat/cf-options-5.4.php';
    }

    // Save old CloudFlare settings.
    if ((isset($newvalue['cloudflare_auto_settings'], $oldvalue['cloudflare_auto_settings']) && $newvalue['cloudflare_auto_settings'] !== $oldvalue['cloudflare_auto_settings'] && $newvalue['cloudflare_auto_settings'] === 1) && (int) get_rocket_option('do_cloudflare') > 0 && phpversion() >= '5.4') {
...
function rocket_handle_settings_import($file_import, $filename_prefix, $inputs)
{
    ...
    if ($file_data['type'] !== 'text/plain' && $file_data['type'] !== 'application/json') {
    ...
    } elseif ($file_data['type'] === 'application/json') {
