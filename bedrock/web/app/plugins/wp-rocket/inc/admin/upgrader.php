...
function rocket_upgrader()
{
    ...
    elseif ($actual_version !== WP_ROCKET_VERSION) {
    ...
    if (! rocket_valid_key() && current_user_can(apply_filters('rocket_capacity', 'manage_options')) &&
        (! isset($_GET['page']) || $_GET['page'] !== 'wprocket')) {
...
function rocket_new_upgrade($wp_rocket_version, $actual_version)
{
    ...
    if (version_compare($actual_version, '2.8.21', '<') && version_compare(phpversion(), '5.4') >= 0) {
        $options = get_option(WP_ROCKET_SLUG);
        if ($options['do_cloudflare'] > 0 && $options['cloudflare_domain'] !== '') {
    ...
    if (version_compare($actual_version, '2.11.2', '<')) {
        ...
        if (! isset($options['wl_plugin_URI']) || $options['wl_plugin_URI'] === 'http://www.wp-rocket.me') {
            $options['wl_plugin_URI'] = 'https://wp-rocket.me';
            $update = true;
        }

        if (! isset($options['wl_author']) || $options['wl_author'] === 'WP Rocket') {
            $options['wl_author'] = 'WP Media';
            $update = true;
        }

        if (! isset($options['wl_author_URI']) || $options['wl_author_URI'] === 'http://www.wp-rocket.me') {
