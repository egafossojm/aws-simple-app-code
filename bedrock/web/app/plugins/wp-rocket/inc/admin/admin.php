...
{
    if ($plugin_file === 'wp-rocket/wp-rocket.php') {
...
function rocket_dismiss_boxes($args)
{
    ...
    if (isset($args['box'], $args['_wpnonce'])) {
        ...
        if ($args['box'] === '__rocket_imagify_notice') {
...
function rocket_rollback()
{
    ...
    if ($transient === false) {
        ...
        wp_die(
            // translators: %s is the plugin name.
            '', sprintf(__('%s Update Rollback', 'rocket'), WP_ROCKET_PLUGIN_NAME), [
                'response' => 200,
            ]
        );
    }
...
function rocket_add_imagify_api_result($result, $action, $args)
{
    ...
    if ($args->browse !== 'featured' && $args->browse !== 'recommended' && $args->browse !== 'popular') {
        return $result;
    }

    if (! isset($result->info['page']) || $result->info['page'] > 1) {
        return $result;
    }
...
function rocket_send_analytics_data()
{
    ...
    if (get_transient('rocket_send_analytics_data') === false) {
