...
class Minify_HTML
{
    ...
    {
        return (strpos($m[1], '[') === 0 || strpos($m[1], '<![') !== false || strpos($m[1], 'esi') === 0 || strpos($m[1], 'noindex') === 0 || strpos($m[1], '/noindex') === 0 || strpos($m[1], 'start_content') === 0 || strpos($m[1], 'end_content') === 0)
            ? $m[0]
            : '';
    }
    ...
    protected function _removeCdata($str)
    {
        ...
        if (strpos($str, '<![CDATA[') !== false) {
            $data['content'] = str_replace(['/* <![CDATA[ */', '/* ]]> */'], '', $str);
            $data['cdata'] = true;
