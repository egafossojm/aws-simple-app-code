...
class Rocket_Critical_CSS
{
     ...
     * @since 2.11
     *
     * @var object Background Process instance.
     */
    public $process;
    ...
    {
        add_action('admin_post_rocket_generate_critical_css', [$this, 'init_critical_css_generation']);
        add_action('update_option_'.WP_ROCKET_SLUG, [$this, 'generate_critical_css_on_activation'], 11, 2);
        add_action('update_option_'.WP_ROCKET_SLUG, [$this, 'stop_process_on_deactivation'], 11, 2);

        if (get_rocket_option('async_css')) {
            add_action('switch_theme', [$this, 'process_handler']);
        }

        add_action('admin_notices', [$this, 'critical_css_generation_running_notice']);
        add_action('admin_notices', [$this, 'critical_css_generation_complete_notice']);
        add_action('admin_notices', [$this, 'warning_critical_css_dir_permissions']);
        add_action('wp_head', [$this, 'insert_load_css'], PHP_INT_MAX);
        add_action('wp_head', [$this, 'insert_critical_css'], 1);
        add_filter('rocket_buffer', [$this, 'async_css'], 15);
    ...
    {
        if (! empty($_POST[WP_ROCKET_SLUG]) && isset($old_value['async_css'], $value['async_css']) && ($old_value['async_css'] !== $value['async_css']) && (int) $value['async_css'] === 1) {
            $this->process_handler();
        }
    }
    ...
    {
        if (! empty($_POST[WP_ROCKET_SLUG]) && isset($old_value['async_css'], $value['async_css']) && ($old_value['async_css'] !== $value['async_css']) && (int) $value['async_css'] === 0) {
            if (method_exists($this->process, 'cancel_process')) {
                $this->process->cancel_process();
            }
    ...
    public function get_public_post_types()
    {
         ...
         * @return array
         */
        $excluded_post_types = apply_filters('rocket_cpcss_excluded_post_types', []);
    ...
    public function get_public_taxonomies()
    {
         ...
         * @return array
         */
        $excluded_taxonomies = apply_filters('rocket_cpcss_excluded_taxonomies', [
            'post_format',
    ...
    public function insert_critical_css()
    {
        ...
        if (in_array($_SERVER['REQUEST_URI'], get_rocket_option('cache_reject_uri', []), true)) {
            return;
        }
        ...
        if ($current_page_critical_css === 'fallback') {
