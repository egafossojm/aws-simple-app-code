    ...
    protected function task($item)
    {
        $response = wp_remote_post(
            ...
            [
                 ...
                 */
                'body' => apply_filters('rocket_cpcss_job_request', [
                    'url' => $item['url'],
                ], $item),
            ]
        );
        ...
        while ($job_data = $this->get_critical_path($data->data->id)) {
            ...
            if ($job_data->data->state === 'complete') {
                $transient = get_transient('rocket_critical_css_generation_process_running');
                ...
                $critical_css_content = wp_kses($job_data->data->critical_path, ["\'", '\"']);
