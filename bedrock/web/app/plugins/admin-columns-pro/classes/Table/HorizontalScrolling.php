...
class HorizontalScrolling implements AC\Registrable
{
    ...
    public function update_table_option_overflow()
    {
        ...
        $this->preferences()->set($list_screen->get_storage_key(), filter_input(INPUT_POST, 'value') === 'true');
    ...
    public function register_screen_option($table)
    {
        ...
        $check_box->set_id('acp_overflow_list_screen_table')
            ->set_options(['yes' => __('Horizontal Scrolling', 'codepress-admin-columns')])
    ...
    {
        wp_enqueue_style('ac-table-screen-option', ACP()->get_url().'assets/core/css/table-screen-options.css', [], ACP()->get_version());
        wp_enqueue_script('ac-table-screen-option', ACP()->get_url().'assets/core/js/table-screen-options.js', [], ACP()->get_version());
    }
