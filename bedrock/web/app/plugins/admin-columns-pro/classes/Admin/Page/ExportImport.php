 ...
 * @since 1.4.6.5
 */
class ExportImport extends Page implements Registrable
{
    ...
    public function handle_import()
    {
        ...
        foreach ($columndata as $type => $_data) {
            ...
            elseif ($layout = $layouts->create($default_layout_data)) {
    ...
    private function get_export_multiselect_options()
    {
        ...
        foreach (AC()->get_list_screens() as $list_screen) {
            ...
            if ($this->export_single_layouts()) {
                ...
                } elseif ($list_screen->get_settings()) {
     ...
     * @return string
     */
    private function get_json_export_string($types = [])
    {
        if (empty($types)) {
            return false;
        }
     ...
     * @return bool|string
     */
    private function get_php_export_string_by_types($types = [])
    {
    ...
    {
        wp_enqueue_style('acp-export-import', ACP()->get_url().'assets/core/css/export-import.css', [], ACP()->get_version());
        wp_enqueue_script('acp-export-import', ACP()->get_url().'assets/core/js/export-import.js', ['jquery'], ACP()->get_version());
        wp_enqueue_script('acp-export-import-multi-select', ACP()->get_url().'assets/core/js/jquery.multi-select.js', ['jquery'], ACP()->get_version());
    }
									...
									<?php foreach ($groups as $group_key => $group) { ?>
										<optgroup label="<?php echo esc_attr($group_key); ?>">
											<?php foreach ($group as $key => $label) { ?>
												<option value="<?php echo esc_attr($key); ?>"<?php selected(array_search($key, $this->get_exported_types()) !== false); ?>>
