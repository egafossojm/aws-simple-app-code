...
class Columns implements AC\Registrable
{
    public function register()
    {
        // Init
        add_action('ac/settings/list_screen', [$this, 'set_layout_on_settings_screen']);

        // Requests
        add_action('ac/settings/handle_request', [$this, 'handle_request']);
        add_action('ac/restore_all_columns', [$this, 'restore_all']);

        // HTML
        add_action('ac/settings/sidebox', [$this, 'settings']);
        add_action('ac/settings/after_title', [$this, 'menu']);
        add_action('ac/settings/sidebox', [$this, 'layout_help']);

        add_filter('ac/settings/list_screen_message_label', [$this, 'add_layout_to_label'], 10, 2);
        add_filter('ac/read_only_message', [$this, 'read_only_message'], 10, 2);
        add_action('ac/settings/scripts', [$this, 'admin_scripts']);

        // Ajax
        add_action('wp_ajax_acp_layout_get_users', [$this, 'ajax_get_users']);
        add_action('wp_ajax_acp_update_layout', [$this, 'ajax_update_layout']);
        add_action('wp_ajax_acp_update_layout_order', [$this, 'ajax_update_layout_order']);
    }
        ...
        switch (filter_input(INPUT_POST, 'acp_action')) {
            ...
            case 'create_layout':
                ...
                if (! $layouts->get_layouts()) {
                    $layouts->create(['name' => __('Original', 'codepress-admin-columns')], true);
                }
    ...
    public function admin_scripts()
    {
        ...
        wp_enqueue_style('acp-layouts', $this->get_assets_url().'css/layouts.css', [], ACP()->get_version());

        // Select2
        wp_enqueue_style('ac-select2');
        wp_enqueue_script('ac-select2');
        wp_enqueue_script('acp-layouts', $this->get_assets_url().'js/layouts.js', ['ac-select2'], ACP()->get_version());
    ...
    public function settings(ListScreen $list_screen)
    {
		...
		<div class="sidebox layouts" data-type="<?php echo $list_screen->get_key(); ?>">
			...
			<div class="item new">
				<form method="post" action="<?php echo esc_attr(add_query_arg(['list_screen' => $list_screen->get_key()], ac_get_admin_url())); // without layout id?>">
    ...
    public function ajax_update_layout()
    {
        ...
        $layout = ACP()->layouts($list_screen)->update($data['layout_id'], [
    ...
    {
        return esc_url(add_query_arg(['layout_id' => $layout], $list_screen->get_edit_link()));
    }
    ...
    public function input_rows($attr_id, $layout = false, $is_disabled = false)
    {
        ...
        $view = new AC\View([
            ...
            'select_roles' => $this->select_roles($attr_id, $layout ? $layout->get_roles() : [], $is_disabled),
            'select_users' => $this->select_users($attr_id, $layout ? $layout->get_users() : [], $is_disabled),
        ]);
     ...
     */
    private function select_roles($attr_id, $current_roles = [], $is_disabled = false)
    {
     ...
     * @return AC\Form\Element\MultiSelect
     */
    private function select_users($attr_id, $user_ids = [], $is_disabled = false)
    {
        $options = [];
