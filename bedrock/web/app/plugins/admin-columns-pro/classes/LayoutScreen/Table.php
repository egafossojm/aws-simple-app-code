...
class Table implements Registrable
{
    ...
    {
        add_action('ac/table/list_screen', [$this, 'set_current_layout'], 9); // Early priority
    ...
    public function set_current_layout($list_screen)
    {
        ...
        elseif ($layouts->get_layout_by_id(null)) {
    ...
    {
        return add_query_arg(['layout' => $id], $link);
    }
    ...
    {
        wp_enqueue_script('acp-layouts', ACP()->get_url().'assets/core/js/layouts-listings-screen.js', ['jquery'], ACP()->get_version());
        wp_enqueue_style('acp-layouts', ACP()->get_url().'assets/core/css/layouts-listings-screen.css', [], ACP()->get_version());
    }
}
