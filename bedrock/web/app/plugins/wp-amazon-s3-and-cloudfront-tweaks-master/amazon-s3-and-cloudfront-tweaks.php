...
class Amazon_S3_and_CloudFront_Tweaks
{
    ...
    public function __construct()
    {
         ...
         */
        add_filter('as3cf_local_domains', [$this, 'local_domains'], 10, 1);
    ...
    public function local_domains($domains)
    {
        ...
        $domains[] = 'avatar.tcmediasaffaires.com';
        $domains[] = 'cumulus.finance-investissement.com';
        $domains[] = 'cumulus.investmentexecutive.com';
        $domains[] = 'cumulus.conseiller.ca';
        $domains[] = 'cumulus.advisor.ca';
        $domains[] = 'cumulus.avantages.ca';
        ...
        if (is_multisite()) {
            ...
            if ($blog_details !== false && ! in_array($blog_details->domain, $domains)) {
    ...
    {
        $domain_site = get_blog_details(get_current_blog_id())->domain;
        // Example changes domain to another CDN configured for dedicated movies bucket.
        if (strpos($url, 's3-ca-central-1.amazonaws.com') !== false) {
            error_log($url);
            $url_parts = explode('/', $url);
            $url_parts[2] = get_blog_details(get_current_blog_id())->domain;
            unset($url_parts[3]);
            //unset($url_parts[10]);
    ...
    public function get_attached_file_copy_back_to_local($copy_back_to_local, $file, $attachment_id)
    {
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'some-plugin-action') {
    ...
    public function cloudfront_path_parts($path_parts = [], $domain = '')
    {
        ...
        if ($domain === 'cdn.example.com' && count($path_parts) > 1 && $path_parts[0] === 'media') {
            unset($path_parts[0]);
        }
