...
class Amazon_S3_And_CloudFront_Assets_Pull extends Amazon_S3_And_CloudFront_Pro
{
    ...
    public function init($plugin_file_path)
    {
        ...
        add_action('rest_api_init', [$this->domain_check_controller, 'register_routes']);
        add_filter('wp_resource_hints', [$this, 'register_resource_hints'], 10, 2);

        // UI Setup filters
        add_action('as3cf_plugin_load', [$this, 'load_settings_page']);
        ...
        add_action('admin_notices', [$this, 'display_activated_notice']);
        add_action('network_admin_notices', [$this, 'display_activated_notice']);

        // Ajax
        add_action('wp_ajax_'.$this->action_key('check_assets_domain'), [$this, 'ajax_check_assets_domain']);
        add_action('wp_ajax_'.$this->action_key('dismiss_activated_notice'), [$this, 'ajax_dismiss_activated_notice']);

        // Serve files
        add_filter('style_loader_src', [$this, 'rewrite_src'], 10, 2);
        add_filter('script_loader_src', [$this, 'rewrite_src'], 10, 2);
        add_filter('as3cf_get_asset', [$this, 'rewrite_src']);
    }
     ...
     * @param  array  $data
     */
    protected function save_domain_check_result($success, $data = [])
    {
    ...
    {
        $this->enqueue_style('as3cf-assets-pull-styles', 'assets/css/styles', ['as3cf-styles']);
        $this->enqueue_script('as3cf-assets-pull-script', 'assets/js/script', ['jquery', 'underscore']);

        if (! $this->get_setting('domain')) {
            delete_site_transient('as3cf_assets_pull_last_checked');
        }

        wp_localize_script('as3cf-assets-pull-script', 'as3cf_assets_pull', [
            'strings' => [
    ...
    {
        if ($relation_type === 'dns-prefetch' && $this->should_rewrite_urls()) {
     ...
     * @return string
     */
    public function capture_view($view, $args = [])
    {
        ob_start();
     ...
     * @param  array  $data
     */
    public function render_setting($setting_key, $data = [])
    {
    ...
    public function get_setting($key, $default = '')
    {
        ...
        if ($key == 'rewrite-urls' && ! isset($settings[$key])) {
            $value = 0;
        }

        if ($key == 'force-https' && ! isset($settings[$key])) {
            $value = 0;
        }
    ...
    protected function get_cloudfront_setup_steps()
    {
        ...
        $initial_setup_step = $this->get_settings() === false ? 'start' : '';
    ...
    public function display_activated_notice()
    {
        ...
        $message .= sprintf(
            ...
            $this->get_plugin_page_url(['cloudfront_setup_step' => 'start'])
        );
        $message .= $this->capture_view('script/inline-dismiss', [
            'localize_data' => [
