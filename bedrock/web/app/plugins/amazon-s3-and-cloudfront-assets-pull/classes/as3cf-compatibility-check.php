...
if (! class_exists('AS3CF_Compatibility_Check')) {
    ...
    class AS3CF_Compatibility_Check
    {
         ...
         */
        public function __construct($plugin_name, $plugin_slug, $plugin_file_path, $parent_plugin_name = null, $parent_plugin_slug = null, $parent_plugin_required_version = null, $parent_plugin_filename = null, $deactivate_if_not_compatible = false, $parent_plugin_url = null)
        {
         ...
         * @return string
         */
        public function get_parent_plugin_class()
        {
         ...
         * @return string
         */
        public function get_parent_plugin_filename()
        {
            if (! is_null($this->parent_plugin_slug)) {
         ...
         * @return string
         */
        public function get_parent_plugin_basename()
        {
            if (! is_null($this->parent_plugin_slug)) {
         ...
         * @return string
         */
        public function get_parent_plugin_url()
        {
            if (! is_null($this->parent_plugin_slug)) {
        ...
        public function get_plugin_action_url($action, $basename = null)
        {
            ...
            if (in_array($action, ['upgrade', 'install'])) {
                $page = 'update';
                $action .= '-plugin';
            }
         ...
         * @return bool
         */
        public function is_parent_plugin_enabled()
        {
            ...
            if (method_exists($class, 'is_compatible') && ! call_user_func([$class, 'is_compatible'])) {
         ...
         * @return bool
         */
        public function is_parent_plugin_at_version($version)
        {
        ...
        public function check_capabilities()
        {
            if (is_multisite()) {
                ...
                $caps = ['activate_plugins', 'update_plugins', 'install_plugins'];
                foreach ($caps as $cap) {
        ...
        public static function is_installing_or_updating_plugins()
        {
            ...
            if ($pagenow === 'update.php' && isset($_GET['action']) && $_GET['action'] === 'install-plugin') {
                // We are installing a plugin
                self::$is_installing_or_updating_plugins = true;
            }

            if ($pagenow === 'plugins.php' && isset($_POST['action'])) {
                $action = $_POST['action'];
            ...
            if ($pagenow === 'update-core.php' && isset($_GET['action']) && $_GET['action'] === 'do-plugin-upgrade') {
        ...
        {
            if (! in_array(basename($plugin), ['amazon-s3-and-cloudfront-pro.php', 'wordpress-s3.php'])) {
                return false;
            }
            ...
            foreach ($active_plugins as $basename) {
                if (strpos($basename, $plugin_to_deactivate) !== false) {
