...
if (! class_exists('acf_field_icon_picker')) {
    ...
    class acf_field_icon_picker extends acf_field
    {
            ...
            foreach ($shown as $name => $label) {
                ...
                switch ($name) {
                    ...
                    case 'media_library':
                        ...
                        if (in_array($field['value']['type'], ['media_library', 'dashicons'], true) && ! empty($field['value']['value'])) {
                            $button_style = '';
                        }
                        ?>
								<button
								...
								>
									<div class="acf-icon-picker-media-library-preview-img" style="<?php echo esc_attr($field['value']['type'] !== 'media_library' ? 'display: none;' : ''); ?>">
										<?php
                        ...
                        ?>
									...
									<div class="acf-icon-picker-media-library-preview-dashicon" style="<?php echo esc_attr($field['value']['type'] !== 'dashicons' ? 'display: none;' : ''); ?>">
         ...
         * @since 6.3
         *
         * @param  int  $valid  The current validation status.
        ...
        public function validate_rest_value($valid, $value, $field)
        {
            ...
            if (! empty($value['type']) && $value['type'] === 'media_library') {
                ...
                if (! is_int($value['value']) || get_post_type($value['value']) !== 'attachment') {
