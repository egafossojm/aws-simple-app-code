...
if (! class_exists('ACF_Field_User')) {
    ...
    class ACF_Field_User extends ACF_Field
    {
        ...
        public function initialize()
        {
            ...
            acf_add_filter_variations('acf/fields/user/query', ['name', 'key'], 1);
            acf_add_filter_variations('acf/fields/user/result', ['name', 'key'], 2);
            acf_add_filter_variations('acf/fields/user/search_columns', ['name', 'key'], 3);
            add_filter('acf/conditional_logic/choices', [$this, 'render_field_user_conditional_choices'], 10, 3);

            // Add AJAX query.
            add_action('wp_ajax_acf/fields/user/query', [$this, 'ajax_query']);
            add_action('wp_ajax_nopriv_acf/fields/user/query', [$this, 'ajax_query']);
        }
        ...
        public function render_field_user_conditional_choices($choices, $conditional_field, $rule_value)
        {
            ...
            if (! empty($rule_value)) {
                ...
                $choices = [$user_result['id'] => $user_result['text']];
            }
        ...
        public function update_value($value, $post_id, $field)
        {
            ...
            // Bail early if no value.
            if (empty($value)) {
                acf_update_bidirectional_values([], $post_id, $field, 'user');
        ...
        public function ajax_query()
        {
            ...
            // Add query hooks.
            add_action('acf/ajax/query_users/init', [$this, 'ajax_query_init'], 10, 2);
            add_filter('acf/ajax/query_users/args', [$this, 'ajax_query_args'], 10, 3);
            add_filter('acf/ajax/query_users/result', [$this, 'ajax_query_result'], 10, 3);
            add_filter('acf/ajax/query_users/search_columns', [$this, 'ajax_query_search_columns'], 10, 4);
         ...
         * @return void
         */
        public function ajax_query_init($request, $query)
        {
            // Require field and make sure it's a user field.
            if (! $query->field || $query->field['type'] !== $this->name) {
                $query->send(new WP_Error('acf_missing_field', __('Error loading field.', 'acf'), ['status' => 404]));
            }
            ...
            if (! acf_verify_ajax($nonce, $key)) {
                $query->send(new WP_Error('acf_invalid_request', __('Invalid request.', 'acf'), ['status' => 404]));
            }
        }
         ...
         * @return array
         */
        public function ajax_query_args($args, $request, $query)
        {
         ...
         * @return array
         */
        public function ajax_query_search_columns($columns, $search, $WP_User_Query, $query)
        {
         ...
         * @return array
         */
        public function ajax_query_result($item, $user, $query)
        {
         ...
         *
         * @param  bool  $valid  The current validity booleean
