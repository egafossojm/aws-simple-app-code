    ...
    class ACF_Ajax_Local_JSON_Diff extends ACF_Ajax
    {
        ...
        public function get_response($request)
        {
            ...
            if (! acf_current_user_can_admin()) {
                return new WP_Error('acf_not_allowed', __('Sorry, you do not have permission to do that.', 'acf'), ['status' => 403]);
            }
            ...
            if (! in_array($post_type, acf_get_internal_post_types(), true)) {
                return new WP_Error('acf_invalid_post_type', __('Invalid post type selected for review.', 'acf'), ['status' => 404]);
            }
            ...
            if (! $post) {
                return new WP_Error('acf_invalid_id', __('Invalid post ID.', 'acf'), ['status' => 404]);
            }
            ...
            $key = $post['key'];
            if (! isset($files[$key])) {
                return new WP_Error('acf_cannot_compare', __('Sorry, this post is unavailable for diff comparison.', 'acf'), ['status' => 404]);
            }
