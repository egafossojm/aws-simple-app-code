...
if (! class_exists('ACF_Internal_Post_Type')) {
    abstract class ACF_Internal_Post_Type
    {
        ...
        public function __construct()
        {
            ...
            add_action("acf/validate_{$this->hook_name}", [$this, 'translate_post']);

            add_filter('wp_unique_post_slug', [$this, 'apply_unique_post_slug'], 999, 6);
            add_action('wp_untrash_post_status', [$this, 'untrash_post_status'], 10, 3);
        }
        ...
        public function get_raw_post($id = 0)
        {
            ...
            $raw_post['active'] = in_array($post->post_status, ['publish', 'auto-draft']);
         ...
         * @return array
         */
        public function filter_posts($posts, $args = [])
        {
        ...
        public function update_post($post)
        {
            ...
            $_post = $post;
            acf_extract_vars($_post, ['ID', 'key', 'title', 'menu_order', 'fields', 'active', '_valid', '_parent']);
        ...
        public function duplicate_post($id = 0, $new_post_id = 0)
        {
            ...
            if (! $post['ID']) {
                $post['ID'] = wp_insert_post(['post_title' => $post['key']]);
            }
         ...
         * @return array
         */
        public function prepare_post_for_export($post = [])
        {
            // Remove args.
            acf_extract_vars($post, ['ID', 'local', '_valid']);
