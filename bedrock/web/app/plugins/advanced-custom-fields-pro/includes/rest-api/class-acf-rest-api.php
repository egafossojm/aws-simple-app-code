...
class ACF_Rest_Api
{
    ...
    {
        add_filter('rest_pre_dispatch', [$this, 'initialize'], 10, 3);
    ...
    private function get_schema()
    {
        $schema = [
            ...
            'properties' => [],
            'arg_options' => [
                'validate_callback' => [$this, 'validate_rest_arg'],
            ],
        ];
    ...
    public function update_fields($data, $object, $property, $request, $object_sub_type)
    {
        ...
        $field_group_scope = acf_extract_var($data, '_acf_field_group_scope', []);
        ...
        if ($all_fields) {
            ...
            // field search.
            $field_key_map = acf_extract_var($data, '_acf_field_key_map', []);
     ...
     * @return bool
     */
    private function object_type_has_field_group($object_type, $field_group, $location_types = [])
    {
        ...
        foreach ($field_group['location'] as $rule_group) {
            ...
            foreach ($rule_group as $rule) {
                 ...
                 * post type.
                 */
                if ($object_type === 'post' && $rule['param'] === 'post_type') {
                ...
                if ($object_type === 'term' && $rule['param'] === 'taxonomy') {
                    if ($rule['operator'] === '==' && $this->request->object_sub_type !== $rule['value']) {
                        continue;
                    }
                    if ($rule['operator'] === '!=' && $this->request->object_sub_type === $rule['value']) {
                ...
                if (in_array($object_type, ['user', 'comment'])) {
                    $match = true;
                }
            }
     ...
     */
    private function get_field_groups_by_id($object_id, $object_type, $object_sub_type = null, $scope = [])
    {
        ...
        switch ($object_type) {
            ...
            default:
                ...
                if ($child_rest_base && $object_type === 'post') {
