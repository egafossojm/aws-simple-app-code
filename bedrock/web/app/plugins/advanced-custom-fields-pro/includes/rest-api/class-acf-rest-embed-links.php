...
class ACF_Rest_Embed_Links
{
    ...
    private function hook_link_handlers()
    {
        foreach (get_post_types(['show_in_rest' => true]) as $post_type) {
            add_filter("rest_prepare_{$post_type}", [$this, 'load_item_links'], 10, 3);
        }

        foreach (get_taxonomies(['show_in_rest' => true]) as $taxonomy) {
            add_filter("rest_prepare_{$taxonomy}", [$this, 'load_item_links'], 10, 3);
        }

        add_filter('rest_prepare_user', [$this, 'load_item_links'], 10, 3);
    }
