...
if (! class_exists('ACF_Form_User')) {
    ...
    class ACF_Form_User
    {
        ...
        public function __construct()
        {
            ...
            // render
            add_action('show_user_profile', [$this, 'render_edit']);
            add_action('edit_user_profile', [$this, 'render_edit']);
            add_action('user_new_form', [$this, 'render_new']);
            add_action('register_form', [$this, 'render_register']);

            // save
            add_action('user_register', [$this, 'save_user']);
            add_action('profile_update', [$this, 'save_user']);

            // Perform validation before new user is registered.
            add_filter('registration_errors', [$this, 'filter_registration_errors'], 10, 3);
        }
        ...
        public function admin_enqueue_scripts()
        {
            ...
            if (! acf_is_screen(['profile', 'user', 'user-edit', 'profile-network', 'user-network', 'user-edit-network'])) {
                return;
            }
         ...
         * @return void
         */
        public function render_edit($user)
        {
        ...
        public function render($args = [])
        {
            ...
            if (isset($_POST['acf'])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
                add_filter('acf/pre_load_value', [$this, 'filter_pre_load_value'], 10, 3);
            }
...
(function($) {
	...
	if( !$submit.next('.spinner').length ) {
         ...
         * @return mixed
         */
        public function filter_pre_load_value($null, $post_id, $field)
        {
            $field_key = $field['key'];
