...
function acf_get_value($post_id, $field)
{
    ...
    if (empty($field['type']) && empty($field['key'])) {
        ...
        if (apply_filters('acf/prevent_access_to_unknown_fields', false) || ($decoded['type'] === 'option' && $decoded['id'] !== 'options')) {
            $allow_load = false;
        }
    }

    // If we're using a non options_ option key, ensure we have a valid reference key.
    if ($decoded['type'] === 'option' && $decoded['id'] !== 'options') {
 ...
 * @param  int|string  $post_id  The post id.
 * @param  array  $field  The field array.
 * @param  bool  $escape_html  Ask the field for a HTML safe version of it's output.
 * @return mixed
 */
