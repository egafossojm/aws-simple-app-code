    ...
    if (! empty($valid_targets)) {
        ...
        foreach ($valid_targets as $target_field) {
            foreach ($additions as $addition) {
                ...
                update_field($target_field, array_unique(array_merge($current_value, [$item_id])), $addition);
            }

            foreach ($subtractions as $subtraction) {
                $current_value = acf_get_array(get_field($target_field, $subtraction, false));
                update_field($target_field, array_unique(array_diff($current_value, [$item_id])), $subtraction);
            }
        }
...
function acf_get_valid_bidirectional_target_types($object_type)
{
    ...
    switch ($object_type) {
        case 'term':
            $valid_target_types = ['taxonomy'];
            break;
        case 'user':
            $valid_target_types = ['user'];
            break;
        case 'post':
            $valid_target_types = ['relationship', 'post_object'];
            break;
    }
...
{
    $valid_field_types = apply_filters('acf/bidirectional/supported_target_field_types', ['relationship', 'post_object', 'user', 'taxonomy']);
        ...
        foreach ($fields as $field) {
            if (in_array($field['type'], $valid_field_types, true)) {
                if (empty($valid_fields[$field_group['title']])) {
                    $valid_fields[$field_group['title']] = [];
                }
