...
if (! class_exists('ACF_Taxonomy')) {
    class ACF_Taxonomy extends ACF_Internal_Post_Type
    {
        ...
        public function __construct()
        {
            ...
            add_action('acf/init', [$this, 'register_taxonomies'], 6);
        }
        ...
        {
            $taxonomies = $this->get_posts(['active' => true]);
        ...
        public function get_settings_array()
        {
            return [
                ...
                'object_type' => [], // Converted to objects array passed as second parameter.
         ...
         * @param  bool  $escape_labels  Determines if the label values should be escaped.
         * @return array
         */
        public function get_taxonomy_args($post, $escape_labels = true)
        {
            ...
            if (! empty($rest_namespace) && $rest_namespace !== 'wp/v2') {
            ...
            if (! empty($rest_controller_class) && $rest_controller_class !== 'WP_REST_Terms_Controller') {
            ...
            if (! empty($post['capabilities']['manage_terms']) && $post['capabilities']['manage_terms'] !== 'manage_categories') {
                $capabilities['manage_terms'] = (string) $post['capabilities']['manage_terms'];
            }

            if (! empty($post['capabilities']['edit_terms']) && $post['capabilities']['edit_terms'] !== 'manage_categories') {
                $capabilities['edit_terms'] = (string) $post['capabilities']['edit_terms'];
            }

            if (! empty($post['capabilities']['delete_terms']) && $post['capabilities']['delete_terms'] !== 'manage_categories') {
                $capabilities['delete_terms'] = (string) $post['capabilities']['delete_terms'];
            }

            if (! empty($post['capabilities']['assign_terms']) && $post['capabilities']['assign_terms'] !== 'edit_posts') {
            ...
            if ($meta_box === 'custom' && ! empty($post['meta_box_cb'])) {
                $args['meta_box_cb'] = [$this, 'build_safe_context_for_metabox_cb'];

                if (! empty($post['meta_box_sanitize_cb'])) {
                    $args['meta_box_sanitize_cb'] = (string) $post['meta_box_sanitize_cb'];
                }
            } elseif ($meta_box === 'disabled') {
            ...
            if ($rewrite['permalink_rewrite'] === 'no_permalink') {
                $rewrite_enabled = false;
            }

            // Rewrite slug defaults to $taxonomy key.
            if (! empty($rewrite['slug']) && $rewrite['slug'] !== $post['taxonomy'] && $rewrite['permalink_rewrite'] === 'custom_permalink') {
         ...
         * @return string
         */
        public function export_post_as_php($post = [])
        {
            $return = '';
            if (empty($post)) {
                return $return;
        ...
        public function flush_post_cache($post)
        {
            ...
            if (! empty($post['active'])) {
                $post['object_type'] = isset($post['object_type']) ? (array) $post['object_type'] : [];
