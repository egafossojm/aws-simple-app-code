...
if (! class_exists('ACF_Post_Type')) {
    class ACF_Post_Type extends ACF_Internal_Post_Type
    {
        ...
        public function __construct()
        {
            ...
            add_action('acf/init', [$this, 'register_post_types'], 6);
        ...
        {
            foreach ($this->get_posts(['active' => true]) as $post_type) {
        ...
        {
            $post_types = $this->get_posts(['active' => true]);
        ...
        public function validate_post($post = [])
        {
            ...
            foreach ($post as $setting => $value) {
                if (isset($defaults[$setting])) {
                    ...
                    if ($default_type === 'boolean' && in_array($value, ['0', '1'], true)) {
                        $post[$setting] = (bool) $value;
                    }

                    if ($default_type === 'boolean' && in_array($value, ['false', 'true'], true)) {
                        $post[$setting] = ! ($value === 'false');
                    }
                }
            }
        ...
        public function get_post_type_args($post, $escape_labels = true)
        {
            ...
            if (! empty($rest_controller_class) && $rest_controller_class !== 'WP_REST_Posts_Controller') {
            ...
            if ($rename_capabilities) {
                ...
                if (! empty($singular_capability_name) && ! empty($plural_capability_name)) {
                    $capability_type = [$singular_capability_name, $plural_capability_name];
                } elseif (! empty($singular_capability_name)) {
                    $capability_type = $singular_capability_name;
                }

                if ($capability_type !== 'post' && $capability_type !== ['post', 'posts']) {
            ...
            $supports = is_array($post['supports']) ? $post['supports'] : [];
            ...
            if (! empty($post['register_meta_box_cb'])) {
                $args['register_meta_box_cb'] = [$this, 'build_safe_context_for_metabox_cb'];
            }
            ...
            if (! empty($rewrite['slug']) && $rewrite['slug'] !== $post['post_type'] && $rewrite['permalink_rewrite'] === 'custom_permalink') {
        ...
        public function import_cptui_post_type($args)
        {
            ...
            if (isset($args['capability_type'])) {
                if (trim($args['capability_type']) !== 'post') {
