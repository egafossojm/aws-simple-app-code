...
if (! class_exists('acf_pro_updates')) {
    ...
    class acf_pro_updates
    {
        ...
        public function init()
        {
            ...
            if (is_admin()) {
                add_action('in_plugin_update_message-'.acf_get_setting('basename'), [$this, 'modify_plugin_update_message'], 10, 2);
            }
        }
 ...
 * @since   5.4.0
 *
 * @return array|bool $license  The ACF PRO license array on success, or false on failure.
 */
 ...
 * @since 6.2.4
 *
 * @return bool true if the updates page should be hidden as we're not the primary multisite site.
 */
function acf_pro_is_updates_page_visible()
{
    ...
    if (acf_is_multisite_sub_site()) {
        $status = get_blog_option(get_main_site_id(), 'acf_pro_license_status', []);
 ...
 * @since 5.4.0
 *
 * @param  bool  $skip_url_check  Skip the check of the current site url.
 ...
 * @param  bool  $automatic  True if this activation is happening automatically.
...
function acf_pro_get_license_status($force_check = false)
{
    ...
    $status = acf_pro_get_license_option('acf_pro_license_status', []);
    ...
    if ((empty($status) || $force_check || time() > $next_check) && $license) {
        if (! get_transient('acf_pro_validating_license') || $force_check) {
            ...
            if (is_array($response)) {
                ...
                if (! empty($response['code']) && $response['code'] === 'activation_not_found') {
 ...
 * @return array
 */
function acf_pro_parse_license_status($status = [])
{
 ...
 */
function acf_pro_is_license_active($status = [])
{
    if (empty($status)) {
 ...
 */
function acf_pro_is_license_expired($status = [])
{
    if (empty($status)) {
        $status = acf_pro_get_license_status();
    }
    ...
    return in_array($status['status'], ['expired', 'cancelled'], true);
}
 ...
 */
function acf_pro_was_license_refunded($status = [])
{
    if (empty($status)) {
        $status = acf_pro_get_license_status();
    }
 ...
 */
function acf_pro_has_license_url_changed($license = [], $url = '')
{
 ...
 * @return string
 */
function acf_pro_get_manage_license_url($status = [])
{
    if (empty($status)) {
        $status = acf_pro_get_license_status();
    }
 ...
 * @param  bool  $autoload  True if the option should be autoloaded.
 ...
 *
 * @param  string  $option_name  The option name to load.
 * @return bool The result of the deletion request.
 */
 ...
 *
 * @param  array  $status  Optional license status array.
 * @return bool True if legacy multisite, false if not.
 */
function acf_pro_is_legacy_multisite(array $status = [])
{
    if (! is_multisite()) {
        return false;
    }
