 ...
 * @param  bool  $is_ajax_render  Whether or not this is an ACF AJAX render.
 * @return string The block HTML.
 */
...
function acf_enqueue_block_assets()
{
    ...
    wp_enqueue_script('acf-blocks', $blocks_js_path, ['acf-input', 'wp-blocks'], ACF_VERSION, true);
...
function acf_enqueue_block_type_assets($block_type)
{
    ...
    if ($block_type['enqueue_style']) {
        wp_enqueue_style($handle, $block_type['enqueue_style'], [], ACF_VERSION, 'all');
    }

    // Enqueue script.
    if ($block_type['enqueue_script']) {
        wp_enqueue_script($handle, $block_type['enqueue_script'], [], ACF_VERSION, true);
    }
...
function acf_ajax_fetch_block()
{
    ...
    $args['context'] = isset($_REQUEST['context']) ? $_REQUEST['context'] : []; //phpcs:ignore -- requires auth; designed to contain unescaped html.
 ...
 * @param  bool  $force  If we should generate a new block ID even if one exists.
 * @return string A block ID.
 */
function acf_get_block_id($attributes, $context = [], $force = false)
{
 ...
 * @param  bool  $using_defaults  True if the block is currently being generated with default values. Default false.
 * @param  bool  $use_post_data  True if we should validate the POSTed data rather than local meta values. Default false.
 * @param  bool  $on_load  True if we're validating as part of a render. This is essentially the same as a first load. Default false.
 ...
 * @return array|bool An array containing the validation errors, or false if there are no errors.
 */
 ...
 * @param  bool  $using_defaults  True if this is the first load of the block, when special validation may apply.
 * @return array|bool An array containing the validation errors, or false if there are no errors.
 */
function acf_validate_block_from_local_meta($block_id, $field_objects, $using_defaults = false)
{
    ...
    foreach ($field_objects as $field) {
        ...
        if ($using_defaults || $using_loaded_meta) {
            ...
            if ($field['required'] && in_array($field['type'], ['radio', 'button_group', 'select'], true) && ! $field['allow_null']) {
                continue;
            }
        }
 ...
 * @param  int  $post_id  The ID of the post to retrieve meta from.
 * @return array
 */
