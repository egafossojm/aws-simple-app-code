    ...
    class acf_field_repeater extends acf_field
    {
        ...
        public function update_value($value, $post_id, $field)
        {
            ...
            if (! empty($field['pagination']) && did_action('acf/save_post') && ! isset($_POST['_acf_form'])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Value not used.
                ...
                $old_rows = is_array($old_rows) ? $old_rows : [];
                $edited_rows = [];
                $deleted_rows = [];
                $reordered_rows = [];
                $new_rows = [];
                ...
                foreach ($value as $key => $row) {
                    ...
                    if (strpos($key, 'row') === false) {
                        ...
                        if ($reordered_row_num !== false && $reordered_row_num <= $old_value) {
        ...
        public function prepare_field_for_import($field)
        {
            ...
            if (! empty($field['sub_fields'])) {
                ...
                return array_merge([$field], $sub_fields);
            }
        ...
        public function ajax_get_rows()
        {
            ...
            if (! acf_verify_ajax($args['nonce'], $args['field_key'])) {
                $error = ['error' => __('Invalid nonce.', 'acf')];
                wp_send_json_error($error, 401);
            }

            if ($args['field_name'] === '' || $args['field_key'] === '') {
                $error = ['error' => __('Invalid field key or name.', 'acf')];
            ...
            if (! $field || ! $post_id) {
                $error = ['error' => __('There was an error retrieving the field.', 'acf')];
                wp_send_json_error($error, 404);
            }
