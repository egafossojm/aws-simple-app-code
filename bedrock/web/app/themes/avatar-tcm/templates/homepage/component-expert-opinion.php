...
*/
$i = 0;
$slugExpertOption = 'expertpanel_';
$idExpertOption = get_cat_ID('Expert Panel');
		...
		<?php if ($avatar_columnist_page_obj) { ?>
			<h2 class="bloc-title">
				<a class="bloc-title__link" href="<?php echo esc_url(get_page_link($avatar_columnist_page_obj->ID)); ?>"
					   ...
					   <?php echo get_the_title($avatar_columnist_page_obj->ID); ?> CIR
				</a>
		...
		<div class="row equal-col component-inside-track__listing regular-listing">
			...
			<?php while ($avatar_columnists->have_posts()) {
			    ...
			    $articles_args = [
			        'post_type' => 'post',
			        'posts_per_page' => 1,
			        'category_name' => $slugExpertOption,
			        'meta_query' => [
			            [
			                'key' => 'imported_site_origin',
			                'value' => 'CIR',
			                'compare' => '=',
			            ],
			            [
