...
<?php while (have_posts()) {
    ...
    $singleMainCategoryFieldCatSlug = $singleMainCategoryFieldCat->slug ?? null;
    ...
    $url_category = isset($category[0]) ? get_category_link($category[0]) : null;
    ...
    switch ($site_id) {
        ...
        case 6:
            ...
            $aside_sponsor_and_region = isset($category[0]) && ($idGestionnaireEnDirect == $category[0]) || (isset($category[1]) && ($idGestionnaireEnDirect == $category[1]));
            $site_url = get_blog_details($site_id)->siteurl;
    ...
    ?>
			...
			<?php if (is_singular('microsite') || (($singleMainCategoryField && ($singleMainCategoryFieldCatSlug == 'conference-coverage-canadian-investment-review') || $singleMainCategoryFieldCatSlug == 'conference-coverage'))) { ?>
				<div>
					...
					<?php } else { ?>
						<div class="row equal-col-md">
							...
							<?php } else { ?>
								<?php if (has_post_thumbnail()) { ?>
									...
									<figure class="main post-thumbnail">
										<?php the_post_thumbnail('post-featured-image', ['class' => 'img-responsive']); ?>
							...
							<?php } else { ?>
								...
								<?php
							    if (isset($avatar_wpseo['company_logo'])) { ?>
								<div class="hidden" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
								...
								<?php } ?>
							<?php } ?>
							<?php if ($avatar_wpseo = get_option('wpseo') && ! empty($avatar_wpseo) && isset($avatar_wpseo['company_logo']) && isset($avatar_wpseo['company_name'])) { ?>
                ...
                avatar_include_subscription_module();
							    ...
							    ?>
									...
									<?php } else { ?>
									    ...
									    $avatart_microsite_template = $avatar_article_site_origin === 'CIR' ? 'templates/cir/microsite/component-microsite-cir-block.php' : 'templates/microsite/component-microsite-block.php';
