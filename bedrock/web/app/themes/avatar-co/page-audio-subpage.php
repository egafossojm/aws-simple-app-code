...
$aside_sponsor_and_region = ($idGestionnaireEnDirect == $thiscat->category_parent);

if ($aside_sponsor_and_region) {
    $sponsorthiscat = get_category($idGestionnaireEnDirect);
				...
				<section class="row row--no-margin color_bg_dark_navy podtopbar">
						<div class="col-sm-6  text-content">
							<h1><?php if ($aside_sponsor_and_region) {
							    echo 'Balados Gestionnaires <i>en direct</i>';
							} else {
							...
							<div class="excerpt"><?php if ($aside_sponsor_and_region) {
							    echo category_description($idGestionnaireEnDirect);
							} else {
...
if (! empty($logo)) { ?>
		...
		<section class="row equal-col-md row--no-margin">
			...
			<?php while ($avatar_first_podcast_subcat_query->have_posts()) {
				...
				<div class="row">
					<div class="col-sm-12">
						<h2 class="bloc-title bloc-title"><?php echo get_cat_name($category[0]); ?></h2>
					</div>
				</div>
				<div class="row">
